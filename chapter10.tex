...
For the assessment of multi\-/episodic perceived quality, \citet{moller_single-call_2011} presented the method of defined\-/use, so multiple participants are exposed to the same multi\-/episodic condition by defining the performance as well as when, how, and for what a service should be used.
...
For the experiments, severe but unrealistic reduction in performance were used, \eg, applying \textsc{\lowercase{LPC\=/10}}.
...
Then, no further decrease is observed although the multi\-/judgments remained well above the episodic judgments of degraded episodes, \ie, a saturation effect was observed.
...
If three consecutive \emph{similar} degraded episodes/days occur (\C6{}), then episodic judgment of the first degraded episode/day is set to the average episodic judgments of all prior non-degraded episodes.
...
It seems important to investigate if the observed effects can also be observed for other types of telecommunication services, or are specific to the mainly investigated speech-based service types.

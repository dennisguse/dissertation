...
With regard to multi-episodic \ac{QoE} the gathered data is not sufficient for creation of models, but can be used to evaluate the performance of a model.
...
For the \ac{AoD} service the content was encoded for \ac{HP} in \ac{AAC} (stereo, \unit[44.1]{kHz}, \unit[320]{kbit/s}) and \ac{LP} was encoded with GSM-FR.
...
\begin{figure}
	...
	<<plotE5C0, echo=F, fig.height=3>>=
		ggplot_timeseries_create(subset(timeseries, experiment=="E5" & condition=="P0")) + geom_boxplot(aes(factor(id), y=QU), na.rm=T)
	@
	...
	@
	\caption{Box plot of episodic judgments in experiment \E5.}
...
%ME
\Sexpr{wilcox(IQU~condition, "E5",  c("HP", "LP"), c("P1", "P2"), 15, "AoD"} %NON-SIG
\Sexpr{wilcox(IQU~condition, "E5",  c("HP", "LP"), c("P1", "P2"), 15, "VoD"} %NON SIGN


%ME per id + condition
\Sexpr{kruskal(IQU~id, "E5", c("HP", "LP"), "P1", unique(timeseries[["id"]]), "AoD")}
\Sexpr{kruskal(IQU~id, "E5", c("HP", "LP"), "P2", unique(timeseries[["id"]]), "AoD")} %SIG

\Sexpr{wilcox.pairwise.print(IQU~id, "E5", c("HP", "LP"), "P2", unique(timeseries[["id"]]), "VoD")}
wilcox.pairwise.print(QU~condition, "E1", "HP", unique(timeseries[["condition"]], c(1:3)))

\Sexpr{kruskal(IQU~id, "E5", c("HP", "LP"), "P1", unique(timeseries[["id"]]), "VoD")}

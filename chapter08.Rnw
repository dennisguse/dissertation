...
The goal of \E4{} was the application the defined-use method in a similar usage period to \citet{moller_single-call_2011} and to gather practical knowledge about how to conduct a field experiment.
...
This is, in fact, an issue with regard to the evaluation, \ie, deriving a \ac{MOS}, as desired by applying the defined-use method, because not all participants assigned to a multi\-/episodic condition were exposed to the same  condition.
...
ggplot_timeseries_create(subset(timeseries, experiment=="E4a" & condition=="A1" & service == "VoD" & id != 0)) + geom_boxplot(aes(factor(id), y=QU), na.rm=T) + xlab("Usage episode")
	@
...
The episodic judgments are significantly different between \ac{HP} and \ac{LP} (\Sexpr{wilcox(QU~performance_level, "E5",  c("HP", "LP"), c("P1", "P2"), id, "AoD")}).
...
However, statistical testing omitted due to the small sample size.
...
\paragraph*{Multi-episodic Judgments}
The multi\-/episodic judgments for both services and conditions are shown in .
...
A post-hoc test shows that the multi\-/episodic judgment of the 2nd~day is significantly different to all following multi\-/episodic judgments ($p<0.006$). %\Sexpr{wilcox.pairwise.print(IQU~id, "E5", c("HP", "LP"), "P2", c(2, 5, 8, 11, 14), "VoD")}
Between the two conditions, only the multi\-/episodic judgments of the 2nd~day are significantly different (\Sexpr{wilcox(IQU~condition, "E5",  c("HP", "LP"), c("P1", "P2"), 2, "VoD")}).
...
The multi\-/episodic conditions are similar to the one-session experiments and are thus denoted with the \emph{same} abbreviations.
...
Then, participants were presented typical speech telephony degradations in the same way as in the one-session experiments (\cf{} \autoref{sec:training}).
...
Here, it was required that participants should answer at least 50\% of the questions correctly if they followed the experimental instructions.
Out of the 24~questions, participants answered on average \Sexpr{round(mean(content[["x"]]), 1)} questions ($\sigma = \Sexpr{round(sd(content[["x"]]), 1)}$) correctly.
...
These three conditions are not significantly different with regard to the final multi\-/episodic judgment (\Sexpr{kruskal(IQU~condition, "E6a", c("HP", "LP"), c("4", "5a", "8"), 14)}).

...
\subsubsection{Episodic Judgments}
...
The episodic judgments are significantly different between performance levels (\Sexpr{wilcox(QU~performance_level, "E6a", unique(timeseries[["performance_level"]]), unique(timeseries[["condition"]]))}).
...
\centering
 \begin{tabulary}{\columnwidth}{c|c|C}
	Condition	& \ac{LP} episode(s) 	& multi-episodic judgment (\ac{MOS})\\
	\midrule
...
\centering
 \begin{tabulary}{\columnwidth}{c|c|C}
	...
	\C4			& 4..5			& \Sexpr{mos_iqu_with_sd_by_condition("E6a", "4", 14)}\\
	\hline
	\C8			& 4, 6			& \Sexpr{mos_iqu_with_sd_by_condition("E6a", "8", 14)}\\
	\hline
	\C5			& 5..6			& \Sexpr{mos_iqu_with_sd_by_condition("E6a", "5a", 14)}\\
	\hline
	\hline

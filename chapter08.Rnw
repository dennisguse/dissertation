...
The integration of perceived quality of distinct episodes into a multi-episodic perceived quality often does not cover one session alone, but rather usage episode with one service occurs over several days.
In difference to consecutive usage in one session alone, here the time between two usage episodes is in general longer.
It is, however, so far unknown how and if at all the duration between episodes affect the quality formation process of multi-episodic perceived quality and thus a multi-episodic judgment.
The findings of the conducted experiments on multi-episodic QoE in one session (\cf, \autoref{chap:lab}), can be used as a basis towards longer usage durations.
...
The first two experiments E4 and E5 focused on general feasibility of conducting field experiments on multi-episodic perceived quality with two services.
Experiment E4 focused on reproduction of the findings of \citet{moller_single-call_2011}, whereas experiment E5 focused on more frequent degraded episodes.
Both experiments had a usage period of \unit[14]{days}.
...
In difference to E4 and E5, only a usage period of \unit[7]{days} with one service only was applied.
Following the laboratory experiments the hypotheses \autoref{hypo:number}, \autoref{hypo:position}, and \autoref{hypo:consecutive} were focused in experiment E6.
%Here field experiment refers to an experiment conducted by the participants on their own in their "real" environment over several days, \ie, multiple sessions each with consisting of one episode.
...
In fact, the experiment of \citet{moller_single-call_2011} showed only little, \ie, not significant, variation of multi-episodic judgments between conditions.
...
Second, an additional service to additionally assess integration of perceived quality of two services.
...
The \ac{VoD} needed to be used daily between \unit[1]{pm} and midnight.
As content for the \ac{VoD} service episodes of \emph{Friends Season 3} were used.
Each original episode was cut into two pieces of about 12 to \unit[17]{min} duration and presented for the \ac{VoD} service in the chronological order.

For the speech telephony service the codec G.722 was used for both performance levels.
For \ac{HP} no additional packet-loss was added whereas \ac{LP} was set to additional 5\% of random packet-loss.
...
The multi-episodic perceived quality for each service was assessed after the 4th, 7th, 10th, and 14th day directly after finishing the episode with the \ac{VoD} service.
...
Especially the high number \ac{LP} in the second condition was expected 

\subsection{Participants} %TODO Verify numbers
10 pairs successfully finished the study, 5 per condition, with an average age of 22.9 years.
...
\section{Experiment E5 \ac{VoD} and \ac{AoD} \citep{guse_modelling_2014}}
...
\begin{table}
	...
	\end{tabular}
	\caption{Overview on conditions for the experiments E6a and E6b incl. number of participants per condition.}
	\label{tab:field:participants}
\end{table}

In both experiment a experimental period of \unit[7]{days} was selected.

had a experiment period of \unit[7]{days} 
...
\centering
 ...
 \end{tabulary}
 \caption{\autoref{hypo:number}: \ac{MOS} of multi-episodic judgments after the 6th day for experiment E7.}
...
\centering
 \begin{tabulary}{\columnwidth}{c|c|C}
	...
	C5a 		& 5..6			& \Sexpr{mos_iqu_with_sd_by_condition("E6a", "5a", 14)}\\
 \end{tabulary}
 \caption{\autoref{hypo:position}: \ac{MOS} of multi-episodic judgment after the 6th usage episode for experiment E5.}

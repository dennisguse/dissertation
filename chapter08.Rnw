...
On each of the \unit[6]{days} of the usage period, the provided \ac{AoD} service needed to be used twice per day.
...
Here, performance is varied on a per day basis, \ie, two episodes of the same day are presented with the same performance level.
...
This should enable participants to focus on the content, but limit the effort.
...
Participants could access the \ac{AoD} via Internet using a \textsc{\lowercase{HTML5}}-capable web browser.
...
It must be noted that the multi\-/episodic judgments remain \unit[1]{pt} higher than the episodic judgment of \ac{LP} episodes.
...
Still, in both cases an effect of position is indicated and thus a recency effect might be observable.
...
However, the slight improvement in the judgment of \C4{} compared to \C5{} and \C8{} can also be explained by a recency effect.
This is similar to \EIIa{} and was also indicated in \E5{}.

\subsection{Discussion}
In this experiment three hypotheses were investigated complementing the one-session experiments on multi\-/episodic \ac{QoE}.
...
Presenting three consecutive days did not result in a further decrease.
...
The slight difference between conditions could be the result of a recency effect.
...
It must be noted that in \E6{} effects were observed and indicated as found in the one-session experiments.
...
This is used in the following to create a prediction model for multi\-/episodic judgments based upon episodic judgments.

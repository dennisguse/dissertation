...
\begin{table}[h]
	...
	\begin{tabulary}{\textwidth}{Z{1.1cm}|Z{2.3cm}|Z{3.4cm}|Z{1.0cm}|Z{1.4cm}}
...
It could be shown that the defined-use method could be applied successfully and practical knowledge about conducting a field experiment acquired.
...
The reason for this is unknown and could be not deduced from the experiments.
With regard to the implementation of a prediction model for multi\-/episodic judgments, the the gathered data is not sufficient due to the failure of the speech telephony service and the limited number of multi\-/episodic conditions.
...
The two services were presented in different multi\-/episodic conditions, \ie, \C9{} for \ac{AoD} and \CX{} for \ac{VoD} as well as \CX{} for \ac{AoD} and \C9{} for \ac{VoD}.
...
Significant differences of the episodic judgments between the two conditions are neither observed for \ac{HP} (\Sexpr{wilcox(QU~condition, "E5",  "HP", c("P1", "P2"),id, "AoD")}) nor for \ac{LP} (\Sexpr{wilcox(QU~condition, "E5",  "LP", c("P1", "P2"), id, "AoD")}).
...
However, statistical testing omitted due to the small sample size (see \autoref{img:field:boxplotE5}).
...
For the \ac{AoD} service, no significant differences between multi\-/episodic judgments are observed for \C9{} (\Sexpr{kruskal(IQU~id, "E5", c("HP", "LP"), "P1", id, "AoD")}).
For \CX{}, a significant difference is found (\Sexpr{kruskal(IQU~id, "E5", c("HP", "LP"), "P2", id, "AoD")}).
...
For the \ac{AoD} service, comparing the multi\-/episodic judgments between the two conditions does not indicate large effects.

For the \ac{VoD} service, no significant differences between multi\-/episodic judgments are found for \C9{} (\Sexpr{kruskal(IQU~id, "E5", c("HP", "LP"), "P1", id, "VoD")}).
For \CX{}, significant differences are found (\Sexpr{kruskal(IQU~id, "E5", c("HP", "LP"), "P2", id, "VoD")}).
A post-hoc test shows that the multi\-/episodic judgment of the 2nd~day is significantly different to all following judgments ($p<0.006$). %\Sexpr{wilcox.pairwise.print(IQU~id, "E5", c("HP", "LP"), "P2", c(2, 5, 8, 11, 14), "VoD")}
...
However, the two conditions only show limited variation of multi\-/episodic judgments after the first \ac{LP} episode is presented.
...
That no difference between the two conditions could be observed, indicates that the number of day-wise performance changes does not seem to affect the multi\-/episodic quality formation process and thus suggests that \autoref{hypo:consecutive} is false.
...
As in \E3{} and \E5{}, the audio book Isabel Allende's \emph{City of the Beasts} was used.
...
Here, it was required that participants should answer at least 50\% of the questions correctly and thus assume they followed the experimental instructions.
...
\C3{}, \C5{}, \C6{}, and the reference are significantly different (\Sexpr{kruskal(IQU~condition, "E6a", performance_level, c("3", "5a", "6"), 14, reference=14)}).
...
\autoref{tab:field:hypoconsecutive} shows the episodic judgment and multi-episodic judgments.
...
Especially, the necessary effort for each pair of participants to conduct the episodes together seems to be limiting.
...
It must be noted that in \E6{} effects that were observed or indicated are similar to the one-session experiments.
...
However, a direct comparison of the experiments cannot be conducted due to the differences in the experimental designs, \ie, tasks, service types, and duration of usage episodes.

...
Beside feasibility, \E4{} focused on the reproduction of the results of \citet{moller_single-call_2011}.
...
First, to achieve a larger difference of multi\-/episodic judgments between conditions.
...
Each pair needed to solve two \acp{SCS} per day. %\footnote{For \E4{} all \acp{SCS} defined in ITU-T P.805 \citep{itu-t_recommendation_p.805_subjective_2007} were used and complement by self-developed but similar \acp{SCS}.}
...
For the \ac{VoD} service the video was encoded with H.264 with a resolution of $720x576$ and the audio with \ac{AAC} (\unit[48]{kHz}, \unit[165]{kbit/s}, stereo).
...
Of the \unit[280]{scheduled calls}, 5~were not done and \unit[18]{episodic} judgments missing.
...
The first episode is judged with \Sexpr{mos_qu_with_sd_by_condition("E4a", "HP", c("A0", "A1"), c(1), "VoD")} while the last with \Sexpr{mos_qu_with_sd_by_condition("E4a", "HP", c("A0", "A1"), c(14), "VoD")}.
...
However, the difference is rather smaller than desired as only a reduction of approximately \unit[1]{pt} is introduced.
...
Here, a \ac{VoD} service and an \ac{AoD} service are used.
Similar to \E4{} the services needed to be used in a usage period of \unit[14]{days}.
In this experiment, the impact of an increase of the number of performance changes between usage episodes on multi\-/episodic judgment is investigated.
...
Here meaningful scenes were cut resulting in durations of \unit[8]{min} to \unit[12]{min} per episode.
The audio signal was treated such as the \ac{AoD} service.
The video signal was encoded to the native resolution of the mobile device (\unit[800$x$480]{px} at \unit[4.3]{inch}) with \unit[25]{\acs{FPS}} with H.264 (two-pass).
...
This judgment was done after finishing the daily episode with the \ac{VoD} service.
...
Out of the overall \unit[294]{usage} episodes, 9~were not conducted for the \ac{AoD} service and 13~not for the \ac{VoD} service.
\unit[6]{multi\-/episodic} judgments were missing.
Similar to \E4{} no participant was removed data analysis due to the lack of ground truth and the small size.
In this \subsection{Data Analysis}
...
However, a post-hoc test does not find a significant difference for \CX{} ($p<0.055$). %\Sexpr{wilcox.pairwise.print(IQU~id, "E5", c("HP", "LP"), "P2", c(2, 5, 8, 11, 14), "AoD")}
...
That no difference between the two conditions could be observed, indicates that the number of day-wise performance changes does not seem not to affect the multi\-/episodic quality formation process.

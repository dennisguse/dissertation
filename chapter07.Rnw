<<setup, echo=F, cache=F, results='asis'>>=
...
In the second stage participant were checked for hearing impairments using an audiometer for the frequency \unit[125]{Hz}, \unit[250]{Hz}, \unit[500]{Hz}, \unit[1000]{Hz}, \unit[2000]{Hz}, \unit[4000]{Hz}, and \unit[8000]{Hz} covering the spectrum for wideband.
...
%In fact, in prior work a negative influence on the episodic QoE judgment of a HP episode that follow one or more LP episode(s) is sometimes observed \citep{moller_single-call_2011, guse_macro-temporal_2013}.
In prior work a negative influence on the episodic QoE judgment of HP episodes is observed, when those directly follow LP episode(s) \citep{moller_single-call_2011, guse_macro-temporal_2013}.
Such an effect could neither be observed for any condition of Study~1 nor Study~2.
As such an effect cannot be ruled out, only episodic QoE judgments of usage episodes 1-3 are compared, because non-HP episodes are only presented for 4th to 6th usage episode.
For Study~1 no significant differences between conditions are found ($H(6)=7.9821, p=0.2394$).
However, for Study~2 a significant difference is found ($H(5)=13.3992, p=0.01991$).
The post-hoc test shows a significant difference between condition 4 ($MOS=4.6$) and 6 ($MOS=4.1$) ($p=0.023,\triangle=0.6$, two-tailed).
A detailed analysis did not yield a reason for the difference, and we thus consider this an artifact due to the between-subject design not affecting the hypotheses testing.
Significant differences of episodic QoE for LP episodes is neither observed in Study~1 ($H(6)=7.8509, p=0.2492$) nor Study~2 ($H(5)=3.8765, p=0.5673$).
Episodic QoE judgments of HP and LP are significant different for Study~1 ($W=106877, p=2.2e-16$, one-sided) and Study~2 ($W=109715.5, p=2.2e-16$, one-sided).

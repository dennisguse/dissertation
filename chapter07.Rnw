...
After every 3rd\,episode per service, a multi\-/episodic judgment needed to be conducted on the same scale.
...
In addition, statistical evaluation is conducted.
...
Although \ac{HP} episodes are significantly different between conditions, an analysis did not yield a reason for the difference.
...
However, conducting a post-hoc test shows no significant differences between conditions for episodic judgments of \ac{HP} episodes (${p \geq 0.066}$).
...
For episodic judgments of \ac{LP} episodes, no significant differences between conditions are found (\Sexpr{kruskal(QU~condition, "E2a", "LP")}).
...
It is thus concluded that the duration did not affect the episodic judgment of the single \ac{LP} episode.
...
For two \ac{HP} episodes, a decrease less than \unit[0.5]{pt} is observed in both experiments.
...
For \E1{}, the multi\-/episodic judgments are significantly different between the \C1{}, \CIIa{}, and \C3{} (\Sexpr{kruskal(IQU~condition, "E1", performance_level, c("1", "2a", "3"), c(6))}).
A one-sided post-hoc test shows that \C1{} is significantly different from \CIIa{} and \C3{} (each \Sexpr{wilcox.pairwise.print(IQU~condition, "E1", performance_level, c("1", "2a", "3"), c(6), alternative="less")[1,1]}), but \CIIa{} and \C3{} are not significantly different (\Sexpr{wilcox.pairwise.print(IQU~condition, "E1", performance_level, c("1", "2a", "3"), c(6), alternative="less")[2,2]}).
...
In fact, this effect could for one \ac{LP} episode only observed, if more than one \ac{HP} episode was presented afterwards.
For \EIIa{}, this could only be observed for two \ac{LP} episodes but not for one \ac{LP} episode.
...
The judgment after the 3rd and 9th episode are not significantly different (\Sexpr{wilcox.pairwise.print(QU~id, "E1", performance_level, "5b", c(3, 6, 9), paired=T)[2,1]}).
...
In fact, both conditions do not show a significant difference between the multi\-/episodic judgments after the 3rd and 9th\,episode.
However, for both conditions a small effect of the non-\ac{HP} episodes on the last multi\-/episodic judgment is indicated, as these remain lower than the first multi\-/episodic judgment.
...
Thus, \autoref{hypo:duration} can be accepted, \ie, a neglect of duration is observed, as even doubling the duration did not produce a measurable effect.
...
As the multi\-/episodic judgments of the speech telephony service is not different if a \ac{VoD} service is present or not, it can be concluded that the speech telephony service was assessed based on the  episodes conducted with this service alone.
...
Here, the multi\-/episodic judgments did not decrease further although more \ac{LP} episodes were experienced.
...
This indicates that previous experiences with \ac{HP} episodes were still present for the multi\-/episodic judgments.
...
With regard to consecutive vs. non-consecutive \ac{LP} episodes the results are conclusive (\autoref{hypo:consecutive}).
...
The existence of a peak effect could neither be proved nor disproved. 

...
\autoref{hypo:number}, \ie, increasing the number of \ac{LP} episodes reduces the following multi-episodic judgment, can be investigated by comparing the results of conditions \C3{}, \C5{} (a+b) and \C6{} as well as conditions \CIIa{} and \C4{}.
...
This is evaluated by comparing conditions \CIIa{} versus \CIIb{}, which both provide the 5th usage episode in \ac{LP} but with a different length.
Whereas episodes of condition \CIIa{} are presented in a similar length, the \ac{LP} episode in condition \CIIb{} is doubled in duration compared.
...
\autoref{hypo:strength} can be investigated by comparing condition \C7{}, which presents the 5th episode in \ac{LP} and the 6th episode in \ac{MP}, with condition \C5{} (a+b) and condition \CIIa{} with regard to the multi-episodic judgment after the 6th usage episode.
Those three condition differ only in the performance of the 6th episode presenting it in \ac{HP}, \ac{MP}, and \ac{LP}.
If a peak-effect occurs in the formation process of multi-episodic perceived quality, the result of condition \C7{} should not differ from the result of condition \CIIa{}.
...
For the \ac{VoD} service a \emph{Nexus 7 (2013)} was used.
This device has a \unit[7]{inch} screen with a resolution of $1920x1200$.
...
In \EIIb{} an additional training was included for the \ac{VoD} service.
...
In \E1{} the duration was not controlled as the \acp{SCT} needed to be fulfilled in a two-party conversation resulting in varying duration.
...
\subsection{Content}
In experiment \E1{} six to nine \acp{SCT}, depending on the condition, were used as defined in \cite{itu-t_p.805:_2007}.
...
The actual order of the \acp{SCT} is given in \appendix \autoref{tab:appendix:labsct}.
...
One recording for each episode 1 to 6 of those that have been presented in \ac{HP} was selected for experiment \EIIa{} and \EIIb{}.
...
For the \ac{VoD} service that was to be simulated in \EIIb{}, scenes from one sitcom were taken.

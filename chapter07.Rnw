...
\autoref{hypo:recovery} is investigated with regard to the multi\-/episodic judgment after the 3rd, 6th, and 9th episode.
...
This is either presented in \ac{MP}, \ac{HP}, and \ac{LP}.
...
This service is presented in two conditions to investigate if its multi\-/episodic conditions affect the episodic judgments as well as multi\-/episodic judgments of the speech telephony service.
...
However, \textsc{\lowercase{LPC\=/10}} is useful for the investigation of multi\-/episodic perceived quality, as it allows to create a severe reduction in performance while maintaining intelligibility and thus tasks remain solvable.
...
In fact, G.722 results in higher \ac{MOS} than G.711, but the difference is smaller than between G.711 and \textsc{\lowercase{LPC\=/10}}.
However, for the investigation of \autoref{hypo:strength}, the actual difference between the three performance levels is not important as long as episodic judgments are different.
...
End-to-end delay is an additional factor influencing the perceived quality.
...
These stimuli contained different signal bandwidths (wideband, narrowband), codecs (G.711, G.722, \textsc{\lowercase{GSM\=/FR}}, \textsc{\lowercase{LPC\=/10}}), white noise (fullband, no codec), and random packet-loss (2\%, and 5\%; G.722, Mode~1, \acs{PLC}: zero insertion).
...
In the \emph{fourth stage}, one the multi\-/episodic condition was presented.
...
For the episodic judgments of \ac{LP} episodes, no significant differences between the conditions are found (\Sexpr{kruskal(QU~condition, "E1", "LP", condition)}).
...
For \EIIa{}, episodic judgments of \ac{HP} episodes are significantly different between the conditions (\Sexpr{kruskal(QU~condition, "E2a", "HP")}).
...
%A post-hoc test shows a significant difference between the condition \C4{} and \C6{} (\Sexpr{wilcox.pairwise.print(QU~condition, "E2a", "HP", condition)[5,4]}).
%Again an analysis did not yield a reason for the difference, which seems to be an artifact due the between-subject design.
For episodic judgments of \ac{LP} episodes, no significant differences between the conditions are found (\Sexpr{kruskal(QU~condition, "E2a", "LP")}).
...
\CIIa{} (normal duration) resulted in a \ac{MOS} of $\Sexpr{mos_qu_with_sd_by_condition("E3", "LP", "2a")}$ whereas \CIIb{} (doubled duration) resulted in a \ac{MOS} of $\Sexpr{mos_qu_with_sd_by_condition("E3", "LP", "2b")}$.
...
\begin{table}[h]
	...
	\midrule
	Multi-episodic Judgment & \Sexpr{mos_iqu_with_sd_by_condition("E1", condition, 3)} & \Sexpr{mos_iqu_with_sd_by_condition("E2a", condition, 3)} & \Sexpr{mos_iqu_with_sd_by_condition("E2b", condition, 3, "telephony", sep=" \\par ")}& \Sexpr{mos_iqu_with_sd_by_condition("E2b", condition, 3, "VoD")} & \Sexpr{mos_iqu_with_sd_by_condition("E3", condition, 3)} \\
	\end{tabulary}
\end{table}
...
\begin{table}[h]
	\centering
	\caption[One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:position}]{One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:position}. Reported as \ac{MOS} with standard deviation in brackets.}
...
In this experiment, \C4{} and \CVb{} are significantly different (\Sexpr{wilcox(IQU~condition, "E1", unique(performance_level), c("4", "5b"), c(6), alternative="greater")}).
...
\begin{table}[h!]
 \centering
 \caption[One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:consecutive}.]{One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:consecutive}. Reported as \ac{MOS} with standard deviation in brackets.}
...
For \CVb{}, a paired post-hoc test shows significant differences between all three multi\-/episodic judgments (3rd vs. 9th: \Sexpr{wilcox.pairwise.print(IQU~id, "E1", performance_level, "5b", c(3, 6, 9), paired=T)[1,1]}; 6th vs. 9th: \Sexpr{wilcox.pairwise.print(IQU~id, "E1", performance_level, "5b", c(3, 6, 9), paired=T)[2,2]}; 3rd vs. 9th \Sexpr{wilcox.pairwise.print(IQU~id, "E1", performance_level, "5b", c(3, 6, 9), paired=T)[2,1]}).
...
\begin{table}[h!]
 \centering
 \caption[One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:duration}]{One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:duration}. Reported as \ac{MOS} with standard deviation in brackets.}
...
\begin{table}[h!]
 \centering
 \caption[One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:independent}]{One-session experiments: multi\-/episodic judgment after the 6th~usage episode for \autoref{hypo:independent}. Reported as \ac{MOS} with standard deviation in brackets.}
...
In fact, a saturation was expected to occur if the multi\-/episodic judgments reach the same level as the episodic judgments of \ac{LP} episodes.

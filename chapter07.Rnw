...
attach(timeseries)
participants <- function(study_filter, condition_filter="ALL") {
	if (condition_filter == "ALL") length(unique(subset(timeseries$username, study==study_filter)))
	else length(unique(subset(timeseries$username, study==study_filter & condition==condition_filter)))
}
@
...
\begin{table}
	...
	\midrule
	$\sum$ 	& \Sexpr{participants("E1")}		&	\Sexpr{participants("E2a")}			&	\Sexpr{participants("E2b")} 	& \Sexpr{participants("E3")} \\
...
\subsubsection{Hypothesis H1: Number of LP Episodes}
In H1 it is assumed that increasing the number of LP episodes before a multi-episodic judgment results in a decrease of this judgment.
This hypothesis can be evaluated by comparing the results multi-episodic judgment after the 6th episode between condition C3, C5, and C6, which present one to three LP episodes directly before the multi-episodic judgment (\cf, \autoref{tab:lab:hypothesesComparison}).

Conditions 2 and 4 present one, respectively, two LP episodes followed by one HP episode before the multi-episodic QoE judgment.


\autoref{tab:lab:h1} shows the multi-episodic QoE judgment after the 6th usage episode 
for these five conditions.

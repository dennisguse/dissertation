...
\begin{table}[h]
	...
	\hline
	E1		& Telephony		& Two-party conversation (\ac{SCT})	& 6 and 9	& H1, H2, H3, H5 \\
	\hline
...
H1, \ie, increasing the number of \ac{LP} episodes reduces the following multi-episodic judgment, can be investigated by comparing the results of conditions C3, C5 (a+b) and C6 as well as conditions C2a and C4.
...
This can be investigated by comparing the multi-episodic judgment of conditions C1, C2, and C3 as well as conditions C4 and C5 (a+b) for one and, respectively, two LP usage episodes.
...
H5 can be investigated by comparing condition C7, which presents the 5th episode in \ac{LP} and the 6th episode in \ac{MP}, with condition C5 (a+b) and condition C2a with regard to the multi-episodic judgment after the 6th usage episode.
...
Non-parametric statistical methods are applied, because the assumptions of parametric methods are likely violated especially due to the limited number of samples.

\subsection{Episodic Judgments} 
For both studies each condition was individually checked for inconsistent participants.
A participant is considered inconsistent, if more than two episodic QoE judgments are beyond 1.5 $\times$ \emph{interquartile range}.
...
Then episodic QoE judgments of all conditions are compared to investigate for an influence of the applied between-subject design.
...
As such an effect cannot be ruled out, only episodic QoE judgments of usage episodes 1-3 are compared, because non-HP episodes are only presented for 4th to 6th episode.

%Add per experiment: order effects LP -> HP!

\subsubsection{Experiment E1}
For E1 significant differences between conditions are found (\Sexpr{kruskal(QU~condition, "E1", "HP", unique(timeseries[["condition"]]), c(1:3))}).
A post-hoc test shows a significant difference between condition C4 (MOS=$\Sexpr{mos_qu_with_sd_by_condition("E1", "HP", "4", c(1:3))}$) and C7 (MOS=$\Sexpr{mos_qu_with_sd_by_condition("E1", "HP", "7", c(1:3))}$) ($p=\Sexpr{wilcox.pairwise.print(QU~condition, "E1", "HP", unique(timeseries[["condition"]]), c(1:3))[["p.value"]][7,4]}$, \Sexpr{wilcox(QU~condition, "E1", "HP", c("4", "7"), c(1:3), diffOnly=T)}).
A detailed analysis did not yield a reason for the difference, and it is thus considered that this is an artifact due to the between-subject design not affecting the multi-episodic hypotheses testing.

A significant difference between conditions for episodic judgments for \ac{LP} episodes are observed in E1 (\Sexpr{kruskal(QU~condition, "E1", "LP", unique(timeseries[["condition"]]))}).

\subsubsection{Experiment E2a and E2b}
For E2a significant differences between conditions are found (\Sexpr{kruskal(QU~condition, "E2a", "HP", unique(timeseries[["condition"]]), c(1:3))}).
A post-hoc test shows a significant difference between condition C4 (MOS=$\Sexpr{mos_qu_with_sd_by_condition("E2a", "HP", "4", c(1:3))}$) and C6 (MOS=$\Sexpr{mos_qu_with_sd_by_condition("E2a", "HP", "7", c(1:3))}$) ($p=\Sexpr{wilcox.pairwise.print(QU~condition, "E2a", "HP", unique(timeseries[["condition"]]), c(1:3))[["p.value"]][5,4]}$, \Sexpr{wilcox(QU~condition, "E1", "HP", c("4", "7"), c(1:3), diffOnly=T)}).
For E2a no significant differences are observed for \ac{LP} between conditions (\Sexpr{kruskal(QU~condition, "E2a", "LP", unique(timeseries[["condition"]]))}).

Significant difference are also found between conditions C5a and C5b for E2b for telephony (\Sexpr{wilcox(QU~condition, "E2b", "HP", c("5a0", "5a4"), c(1:3))}), but not for \ac{VoD} service \Sexpr{wilcox(QU~condition, "E2b", "HP", c("0", "4"), c(1:3))}.
Similar to E2a no significant differences are observed for the two conditions between \ac{LP} for telephony (\Sexpr{wilcox(QU~condition, "E2b", "LP", c("5a0", "5a4"))}).

\subsubsection{Experiment E3}
For experiment E3 no significant differences are found for \ac{HP} between conditions (\Sexpr{wilcox(QU~condition, "E3", "HP", unique(timeseries[["condition"]]), c(1:3))}).
Also no significant difference is observed for \ac{LP} between conditions (\Sexpr{wilcox(QU~condition, "E3", "LP", unique(timeseries[["condition"]]))}).

\subsubsection{Experiment E1 vs. E2a}
Comparing the episodic judgments between experiment E1 and E2a allows to investigate a potential difference between the two usage situation, \eg, two-party conversation vs. passive listening.
Episodic judgments are significant different between E1 and E2a for \ac{HP} (\Sexpr{wilcox(QU~experiment, c("E1", "E2a"), "HP", unique(timeseries[["condition"]]), c(1:3))}) as well as \ac{LP} (\Sexpr{wilcox(QU~experiment, c("E1", "E2a"), "LP", unique(timeseries[["condition"]]))}).
...
Comparing episodic QoE judgments between Study~1 and Study~2 significant differences are found for HP ($W=41868, p=0.004675,\triangle=-0.2$, two-tailed) and also for LP ($W=11803.5, p=0.00005542,\triangle=-0.3$, two-tailed).
For HP episodes Study~1 yielded $MOS=4.1$ and Study~2 $MOS=4.3$.
For LP episodes Study~1 yielded $MOS=1.4$ and Study~2 $MOS=1.8$.

Study~1 resulted in lower episodic QoE judgments for both HP and LP than Study~2.
This indicates that the usage situation influences the formation process of episodic QoE, although the effect on episodic QoE judgments is rather small.

\subsubsection{Experiments E2a and Experiments E2b}

\subsubsection{Experiments E3}

\begin{figure} %TODO Add exemplary plot of (C7)
...
%TODO Insert table of all studies by performance level!


\subsubsection{Consistency per Experiment}
HP vs. MP vs. LP
...
\subsubsection{Impact of Usage Situation}


\subsection{Multi-episodic Judgments}
%TODO
In the following the results for multi-episodic QoE are evaluated.
First consistency between conditions is analyzed with regard to the multi-episodic judgment after the 3rd usage episode.
Then the results are evaluated in detail with regard to the hypotheses under investigation.

%TODO TODO TODO
For the multi-episodic QoE judgment after the 3rd usage episode no significant difference between conditions is found for neither E1 ($H(6)=2.5433, p=0.8636$) nor E2a ($H(5)=6.4111, p=0.2682$).
...
With regard to this multi-episodic judgment both studies do not differ significantly ($H(1)=2.7926, p=0.0947$, E1: $MOS=4.2, \sigma=0.7$, E2a: $MOS=4.3, \sigma=0.8$).
This indicates that, as long as only \ac{HP} episodes are presented, neither the between-subject design nor the task affected this multi-episodic judgment.

In the following, the hypotheses under investigation are analyzed for the experiments E1, E2a, E2b, and E3.

\subsubsection{\autoref{hypo:number}: Number of LP Episodes}
...
This hypothesis can be evaluated by comparing the results multi-episodic judgment after the 6th episode between condition C3, C5, and C6, which present one to three \ac{LP} episodes directly before the multi-episodic judgment (\cf, \autoref{tab:lab:hypothesesComparison}).

Conditions C2a and C4 present one, respectively, two LP episodes followed by one HP episode before the multi-episodic QoE judgment.


\autoref{tab:lab:hyponumber} shows the multi-episodic QoE judgment after the 6th usage episode for these five conditions.
...
\centering
 \begin{tabulary}{\columnwidth}{c|c|C}
	...
	\midrule
	C3			& 6				& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "3", 6)}\\
	\hline
	\hline
	C4			& 4..5			& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "4", 6)}\\
	\hline
	C8			& 4, 5			& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "8", 6)}\\
	\hline
	C5a			& 5..6			& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "5a", 6)}\\
	\hline
	\hline
	C1			& 4				& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "1", 6)}\\
 \end{tabulary}
 \caption{\autoref{hypo:position}: \ac{MOS} of multi-episodic judgment after the 6th usage episode for experiment E1 and E2a.}
 \label{tab:lab:hypoposition}
\end{table}

\subsubsection{\autoref{hypo:strength}: Strength of one \ac{LP} Episode}
...
\centering
 \begin{tabulary}{\columnwidth}{c|C|C|C|C}
	Condition	& Episodic Performance			& \multicolumn{3}{c}{Multi-episodic Judgments} \\
				&					 			& 3rd		& 6th		& 9th  \\
	\midrule
	5b			& 4:\ac{HP}, 5:\ac{LP}, 6:\ac{LP}	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "5b", 3)}	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "5b", 6)}	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "5b", 9)}\\
	\hline
	7			& 4:\ac{HP}, 5:\ac{LP}, 6:\ac{MP}	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "7", 3)}	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "7", 6)} & \Sexpr{mos_iqu_with_sd_by_condition("E1", "7", 9)}\\
 \end{tabulary}
 \caption{\autoref{hypo:recovery}:  MOS of multi-episodic QoE after the 3rd, 6th and 9th usage episode for E1.}
...
\centering
 ...
 \end{tabulary}
 \caption{\autoref{hypo:consecutive}: \ac{MOS} of multi-episodic judgment after the 6th usage episode for E3.}
 \label{tab:lab:hypostrength}
\end{table}

\subsubsection{\autoref{hypo:services}: Services are Judged Independent}

\begin{table}[h!]
\centering
 \begin{tabulary}{\columnwidth}{c|C|C}
	Condition	& \multicolumn{2}{c|}{Multi-episodic Judgment per Service} \\
				& \ac{LST} 	& \ac{VoD} \\
	\midrule 
	C0			& \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a0", 6, "telephony")} & \Sexpr{mos_iqu_with_sd_by_condition("E2b", "0", 6, "video")}\\
	\hline
	C4			& \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a4", 6, "telephony")} & \Sexpr{mos_iqu_with_sd_by_condition("E2b", "4", 6, "video")}\\
 \end{tabulary}
 \caption{\autoref{hypo:hypoindependent}: \ac{MOS} of multi-episodic judgment after the 6th usage episode for E3.}
 \label{tab:lab:hypoindependent}
\end{table}

\section{Excurse: Cross-Service Quality}

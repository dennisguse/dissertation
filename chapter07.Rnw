...
As such an effect cannot be ruled out, only episodic \ac{QoE} judgments of usage episodes 1-3 are compared, because non-\ac{HP} episodes are only presented for 4th to 6th episode.

%Add per experiment: order effects LP -> HP!

\subsubsection{Experiment E1}
Investigating differences between the episodic judgments for \ac{HP} episodes (1-3) showed a significant differences (\Sexpr{kruskal(QU~condition, "E1", "HP", unique(timeseries[["condition"]]), c(1:3))}).
A post-hoc test showed a significant difference between condition C4 (\ac{MOS}=$\Sexpr{mos_qu_with_sd_by_condition("E1", "HP", "4", c(1:3))}$) and C7 (\ac{MOS}=$\Sexpr{mos_qu_with_sd_by_condition("E1", "HP", "7", c(1:3))}$) ($p=\Sexpr{wilcox.pairwise.print(QU~condition, "E1", "HP", unique(timeseries[["condition"]]), c(1:3))[["p.value"]][7,4]}$, \Sexpr{wilcox(QU~condition, "E1", "HP", c("4", "7"), c(1:3), diffOnly=T)}).
...
For the episodic judgments of \ac{LP} episoides no significant differences between conditions are found (\Sexpr{kruskal(QU~condition, "E1", "LP", unique(timeseries[["condition"]]))}).

\subsubsection{Experiment E2a and E2b}
The episodic judgments of \ac{HP} episodes (1-3) are significant different between conditions for experiment E2a (\Sexpr{kruskal(QU~condition, "E2a", "HP", unique(timeseries[["condition"]]), c(1:3))}).
A post-hoc test shows a significant difference between condition C4 (\ac{MOS}=$\Sexpr{mos_qu_with_sd_by_condition("E2a", "HP", "4", c(1:3))}$) and C6 (\ac{MOS}=$\Sexpr{mos_qu_with_sd_by_condition("E2a", "HP", "6", c(1:3))}$) ($p=\Sexpr{wilcox.pairwise.print(QU~condition, "E2a", "HP", unique(timeseries[["condition"]]), c(1:3))[["p.value"]][5,4]}$, \Sexpr{wilcox(QU~condition, "E1", "HP", c("4", "7"), c(1:3), diffOnly=T)}).
Again an analysis did not yield a reason for the difference, which seems to be an artifact due the between-subject design.
For episodic judgments of \ac{LP} episodes no difference between conditions is observed (\Sexpr{kruskal(QU~condition, "E2a", "LP", unique(timeseries[["condition"]]))}).

In experiment E2b the episodic judgments of \ac{HP} episodes (1-3) for the telephony service are significant different between conditions C5a and C5b (\Sexpr{wilcox(QU~condition, "E2b", "HP", c("5a0", "5a4"), c(1:3))}). %TODO Redo-study
For the \ac{VoD} service no significant differences between conditions are found (\Sexpr{wilcox(QU~condition, "E2b", "HP", c("0", "4"), c(1:3))}).
For \ac{LP} episodes neither significant difference for the telephony service (\Sexpr{wilcox(QU~condition, "E2b", "LP", c("5a0", "5a4"))}).

\subsubsection{Experiment E3}
In Experiment E3 no significant difference of the episodic judgments of \ac{HP} episodes between the two conditions is observed (\Sexpr{wilcox(QU~condition, "E3", "HP", unique(timeseries[["condition"]]), c(1:3))}).
Also the episodic judgments of \ac{LP} episodes are not significantly different between the two conditions (\Sexpr{wilcox(QU~condition, "E3", "LP", unique(timeseries[["condition"]]))}).
...
Between the two experiments the episodic judgments of \ac{HP} episodes are significant different (\Sexpr{wilcox(QU~experiment, c("E1", "E2a"), "HP", unique(timeseries[["condition"]]), c(1:3))}) as well as for \ac{LP} episodes (\Sexpr{wilcox(QU~experiment, c("E1", "E2a"), "LP", unique(timeseries[["condition"]]))}). %TODO!!!!
...
E1 resulted in lower episodic judgments for \ac{LP} episodes than E2a.
...
For the multi-episodic judgment after the 3rd episode no significant difference between conditions is found for neither in experiment E1 (\Sexpr{kruskal(IQU~condition, "E1", "HP", unique(timeseries[["condition"]]), c(3))}), experiment E2a (\Sexpr{kruskal(IQU~condition, "E2a", "HP", unique(timeseries[["condition"]]), c(3))}), nor E3  (\Sexpr{wilcox(IQU~condition, "E3", "HP", unique(timeseries[["condition"]]), c(3))}).
For experiment E2b neither the multi-episodic judgments for the telephony service (\Sexpr{wilcox(IQU~condition, "E2b", "HP", c("5a0", "5a4"), c(3))} nor the (\Sexpr{wilcox(IQU~condition, "E2b", "HP", c("0", "4"), c(3))}) are significantly different between the conditions.

This indicates that, as long as only \ac{HP} episodes are presented, the between-subject design does affect this multi-episodic judgments.
...
\autoref{tab:lab:hyponumber} shows the multi-episodic QoE judgment after the 6th usage episode for these five conditions evaluated in experiment E1 and experiment E2a.
Conditions C3, C5a, and C6 are significant different for experiment E1 (\Sexpr{kruskal(IQU~condition, "E1", unique(timeseries[["performance_level"]]), c("3", "5a", "6"), c(6))}) as well as experiment E2a (\Sexpr{kruskal(IQU~condition, "E2a", unique(timeseries[["performance_level"]]), c("3", "5a", "6"), c(6))}).


as well as C2a and C4 (\Sexpr{wilcox(IQU~condition, "E1", unique(timeseries[["performance_level"]]), c("2a", "4"), c(6))}).
...
\subsubsection{\autoref{hypo:position}: Impact of Position of \ac{LP} Episode(s)}
...
E1
\Sexpr{kruskal(IQU~condition, "E1", unique(timeseries[["performance_level"]]), c("1", "2a", "4"), c(6))} \\
\Sexpr{wilcox(IQU~condition, "E1", unique(timeseries[["performance_level"]]), c("4", "5a"), c(6))} \\

E2a
\Sexpr{kruskal(IQU~condition, "E2a", unique(timeseries[["performance_level"]]), c("1", "2a", "4"), c(6))} \\
\Sexpr{wilcox(IQU~condition, "E2a", unique(timeseries[["performance_level"]]), c("4", "5a"), c(6))} \\

...
Furthermore, \textsc{\lowercase{LPC\=/10}} can be used as baseline condition for future work, as an patent-free, open-source implementation is available.
...
However, for the investigation of \autoref{hypo:strength}, the actual differences between the three performance levels are not important as long as episodic judgments are different.

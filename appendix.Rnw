...
For the generation of the stimuli the ITU\=/T~STL2009 tools \citep{itu-t_recommendation_g.191_software_2010} and the audio-processing tool \emph{SOX} were used.
...
As no speech codecs were available in PureData, PureData was extended with the speech codecs G.711, G.722~(Mode~1) and LPC\=/10.
...
Those recordings were processed with the ITU\=/T~STL2009 tools \citep{itu-t_recommendation_g.191_software_2010} for coding with G.722 and with SOX for coding with LPC\=/10.
...
On this server the \ac{VoIP} software \emph{Asterisk~10} was installed, which was configured to act as a \ac{SIP} registrar as well as \ac{RTP}\=/Proxy for the \ac{UDP}\=/based media streams.
...
However, for the media streams no counter measures were taken against packet-loss as the used speech codec G.722 (Mode~1) was used with neither \ac{PLC} nor \ac{FEC} on \ac{UDP} connections without retransmission.
...
As the buffering strategy for media content cannot be controlled by a website using standardized HTML5, the preloading was initiated, when the website was loaded while presenting a loading animation of \unit[5]{sec}.
This avoids issues due to initial buffering by providing a constant waiting time.

\chapter{Questionnaires}\label{appendix:questions}
\begin{table}
 \centering

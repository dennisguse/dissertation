...
The systems implemented for experiments \E4{}, \E5{}, and \E6{} were designed for use in field experiments and thus requiring ease of setup, reliability, and robustness
...
The listening-only training conducted in \E1{} was conducted with a tablet computer (\emph{Fujitsu~Stylistic~ST6012}).
...
For the generation of the stimuli the \textsc{\lowercase{ITU\=/T}~\lowercase{STL2009}} tools \citep{itu-t_recommendation_g.191_software_2010} and the audio-processing tool \textsc{\lowercase{SOX}} were used.
...
For the multi\-/episodic assessment of experiment \E1{} a speech telephony system for two-party conversations was required.
...
Transmission of the speech signal between Asterisk and clients in both directions was lossless by using the \emph{L16} codec with a sample rate of \unit[16]{kHz} resulting in a unidirectional net transmission rate of \unit[256]{kbit/s}.
...
On each of the two client computers one \emph{Beyerdynamic~DT~790~Pro} headset connected to a \emph{Edirol~UA\=/25EX} sound card was used for recording and binaural reproduction.
The output on both systems was calibrated to a comfortable listening level.

\paragraph*{Sound card Setup}
Later in \E1{}, the speech telephony system was replaced by a more elaborate system, which provides easier setup that less complex to verify.
...
As no speech codecs were available in PureData, PureData was extended with the speech codecs G.711, G.722~(Mode~1) and \textsc{\lowercase{LPC\=/10}}.
It was verified that the PureData setup provides similar characteristics as the network\=/based setup described above.
...
For \EIIa{} and \E3{} those were connected to the internal sound card of a \emph{Microsoft~Surface~Pro} tablet computer whereas for \EIIb{} those were connected to the internal sound card of the used \emph{Nexus 7 (2013)}.
In all three experiments the sound pressure level was calibrated to \unit[75]{dB20$\mu$Pa} using the \emph{HSM~II.3} with babble noise and the experiments conducted in a sound-proof cabin following \citet{itu-t_recommendation_p.800_methods_1996}.
In \EIIb{} the \ac{VoD} service was presented on the display of the Nexus~7~(2013) showing videos horizontally in full screen.
...
Those recordings were processed with the \textsc{\lowercase{ITU\=/T}~\lowercase{STL2009}} tools \citep{itu-t_recommendation_g.191_software_2010} for coding with G.722 and with \textsc{\lowercase{SOX}} for coding with \textsc{\lowercase{LPC\=/10}}.
...
\E4{} was the first experiment conducted as a field experiment with a usage period of multiple days.
...
For this service one server was installed in the data center of \emph{Technische~Universität~Berlin} reachable by a public IPv4 address.
...
This traffic shaper allows to add network impairments such as packet-loss, delay, and jitter per individual connection, \ie, protocol, port, source address, destination address and if a packet is incoming or outgoing.
On this server the \ac{VoIP} software \emph{Asterisk\,10} was installed, which was configured to act as a \ac{SIP} registrar as well as \acs{RTP}\=/Proxy for the \acs{UDP}\=/based media streams.
...
However, for the media streams no counter measures were taken against packet-loss as the used speech codec G.722 (Mode\,1) was used with neither \acs{PLC} nor \acs{FEC} on \acs{UDP} connections without retransmission.
...
For this experiment it was chosen not to provide any equipment for participants, but rather let participants use their own equipment.
...
Participants were only required to use as web browser either \emph{Google Chrome} or \emph{Chromium} to limit the testing effort for the website.
As the buffering strategy for media content cannot be controlled by a website using standardized \textsc{\lowercase{HTML5}}, the preloading was initiated, when the website was loaded while presenting a loading animation of \unit[5]{s}.

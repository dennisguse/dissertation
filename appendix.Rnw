...
The \textsc{\lowercase{STL2009}} were used for coding G.711 and G.722~(Mode~1), and inserting packet-loss for G.722 (Mode~1).
\textsc{\lowercase{SOX}} was used for coding \textsc{\lowercase{GSM\=/FR}} and \textsc{\lowercase{LPC\=/10}} as well as filtering wideband, narrowband, and white noise.
...
The clients presented only a minimal user interface, consisting of one button for call initiation and hangup, one button to set the current presence status, and a presence status indicator of the remote client.
Call initiation was only possible if both clients set their presence status to available.
The two clients could not communicate directly with each other as the Asterisk server acted as proxy.
Transmission of the speech signal between Asterisk and the clients (both directions) was lossless by using the \emph{L16} codec (clock rate: \unit[16]{kHz}).
Asterisk applied the desired performance levels (codec) by compression the signal and immediately decompressing it before relaying the signal.\footnote{The performance levels were set by using the transcoding capability of Asterisk. On an incoming call coded with L16, Asterisk initiated a call to himself with the desired codec (\ie, G.722, or LPC\=/10). The later call triggered an outgoing call to the callee encoded in L16.}
...
Rather than transmitting the signals via Ethernet, an analogue transmission via audio cables is used.
Here, the \emph{Beyerdynamic~DT~790~Pro} headsets were also used.
...
On the processing computer \emph{PureData}, an open-source audio processing application, was used to modify the speech signals.
As no speech codecs were available in PureData, the application was extended with the speech codecs G.711, G.722~(Mode~1), and \textsc{\lowercase{LPC\=/10}}.
...
Subsequently, the scenes was processed with \emph{FFmpeg}\footnote{The FFmpeg project is available at \url{https://www.ffmpeg.org/}.} to apply with the desired \acs{QP} factor while leaving the audio unchanged.
...
For this service, one server was installed in the data center of \emph{TU~Berlin}.
...
However, for the media streams no counter measures were taken against packet-loss, \ie, neither \acs{PLC} nor \acs{FEC}. 
...
Second, packet-loss was added by Dummynet in addition to packet-loss occurring due to connection issues.
...
Participants were instructed that for using the \ac{VoD} service the \acs{USB} flash drive must be inserted into the computer and remain connected.
...
The \ac{AoD} service was implemented as a website available online.

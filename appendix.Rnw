...
For the generation of stimuli the ITU-T~STL2009 tools \citet{itu-t_g.191_2010} and the audio-processing tool \emph{SOX} were used.
...
For experiment \EIIa{}, and \E3{} those were connected to the internal sound card of a \emph{Microsoft Surface Pro} tablet computer whereas for experiment \EIIb{} those were connected to the internal sound card of the used \emph{Nexus 7 (2013)}.
...
Those recordings were processed with the ITU-T STL2009 tools \citep{itu-t_g.191_2010} for coding with G.722 and with SOX for coding with LPC-10.
For experiment \EIIb{} the videos material was cut with \emph{Adobe Premiere 6} and exported with H.264 (\unit[1280x800]{px}, \unit[10]{Mbit/s}, two-pass coding) and \ac{AAC} (stereo, \unit[448]{kbit/s}).
...
\begin{table}
	...
	\label{appendix:multiepisodicResult}
\end{table}

...
Two \emph{Fujitsu~Lifebook~S761} were running each a custom client based on \emph{PJSIP~2.1}\footnote{PJSIP is an open-source library for \ac{SIP}\=/based \ac{VoIP} (\url{http://www.pjsip.org}).}, which were connected via \ac{SIP} with the server.
...
The network-based setup was replaced later on by a more elaborate system, which provides easier setup and reduces the complexity of verification.
...
For \EIIa{} and \E3{} those were connected to the internal sound card of a \emph{Microsoft~Surface~Pro} tablet computer whereas for \EIIb{} those were connected to the internal sound card of the \emph{Nexus 7 (2013)}.
...
For those three experiments non-degraded recordings of two-party conversations of \E1{} were selected.
Those recordings were processed with the \textsc{\lowercase{ITU\=/T}~\lowercase{STL2009}} tools \citep{itu-t_recommendation_g.191_software_2010} for coding G.722 and with \textsc{\lowercase{SOX}} for coding \textsc{\lowercase{LPC\=/10}}.
...
Afterwards it was processed with \emph{FFmpeg}\footnote{The FFmpeg project is available at \url{https://www.ffmpeg.org/}.} to re-encode with the desired \ac{QP} factor while leaving the audio unchanged.
...
As operating system \emph{FreeBSD\,9.0} was selected, because it provides a built-in firewall with integrated traffic shaper called \emph{Dummynet}~\citep{rizzo_dummynet_1997}.
...
Based on the reliability of the simulation of a \ac{VoD} service for \E4{}, an \ac{AoD} service and \ac{VoD} service were selected for \E5{}, again simulating both services.
For this experiment ten \emph{Samsung~Galaxy~SII~(GT\=/I9210)} mobile phones running \emph{Android~2.3} were available.
This mobile phone has a \unit[4.5]{inch} display with a resolution of \unit[480$x$800]{px}.

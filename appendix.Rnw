...
timeseries$id[timeseries$experiment == "E6a"] = timeseries$id[timeseries$experiment == "E6a"] - 2

timeseries=timeseries[-which(timeseries$experiment %nin% c("E1", "E2a", "E2b", "E3", "E6a")), ]

timeseries$condition_org = timeseries$condition
timeseries$condition = factor(timeseries$condition, labels=c("C1", "C2a", "C2b", "C3", "C4", "C5a", "VoD: C0", "VoD: C4", "C5b", "C6", "C7", "C8"))

attach(timeseries)
@

\chapter{Experimental Setups}\label{appendix:setups}
The systems that were used in the conducted experiments are described, focusing on the media processing and required details to reproduce the technical conditions.
...
For the diotic representation, a pair of \emph{AKG~K\=/271} headphones was connected to the internal sound card of the tablet computer.
...
The \textsc{\lowercase{STL2009}} were used for coding G.711, G.722~(Mode~1), and inserting packet-loss for G.722 (Mode~1), whereas \textsc{\lowercase{SOX}} was used for coding \textsc{\lowercase{GSM\=/FR}}, and \textsc{\lowercase{LPC\=/10}} as well as filtering wideband, narrowband, and white noise.
...
These three computers were connected via Ethernet (CAT\=/5). 
...
Two \emph{Fujitsu~Lifebook~S761} were running each a customized client based on \emph{PJSIP~2.1}\footnote{PJSIP is an open-source library for \ac{SIP}\=/based \ac{VoIP} (\url{http://www.pjsip.org}).}, which were connected via \ac{SIP} with the server.
The clients presented only a minimal user interface consisting of one button for call initiation and hangup, one button to set the current presence status, and an image presenting the presence status of the other client.
Call initiation was only possible if both clients set their presence status to available.
The two clients could not communicate directly with each other as all connections as the Asterisk server acted as proxy.
Transmission of the speech signal between Asterisk and the clients in both directions was lossless by using the \emph{L16} codec (clock rate: \unit[16]{kHz}).
...
On each of the two client computers, one \emph{Beyerdynamic~DT~790~Pro} headset connected to a \emph{Edirol~UA\=/25EX} sound card was used for recording and diotic reproduction.
...
The network-based setup was replaced later by a more elaborate system that provided easier setup and reduced the complexity of verification.
For this system, only one computer without an actual network is used.
Rather than transmitting the signals digitally via Ethernet, an analogue transmission via audio cables is used.
Here, the \emph{Beyerdynamic~DT~790~Pro} headsets were used.
...
On the processing computer \emph{PureData}, an open-source audio processing program, was used to modify the speech signals.
As no speech codecs were available in PureData, the program was extended with the speech codecs G.711, G.722~(Mode~1), and \textsc{\lowercase{LPC\=/10}}.
It was verified that the PureData setup provides similar characteristics.
...
For \EIIb{}, these were connected to the internal sound card of the \emph{Nexus 7 (2013)}.
...
In \EIIb{}, the \ac{VoD} service was presented on the display of the Nexus\,7~(2013), presenting the videos horizontally in full screen.
The brightness of this \unit[7]{inch} display was set to maximum without applying any device related color adaption.

For these three experiments, non-degraded recordings of two-party conversations of \E1{} were selected.
These recordings were processed with the \textsc{\lowercase{ITU\=/T}~\lowercase{STL2009}} tools \citep{itu-t_recommendation_g.191_software_2010} for coding G.722 and with \textsc{\lowercase{SOX}} for coding \textsc{\lowercase{LPC\=/10}}.
For \EIIb{}, the video material was cut with \emph{Adobe Premiere 6} and exported with H.264 (\unit[1280$x$720]{px}, \unit[5]{Mbit/s}, two-pass) and \ac{AAC} (\unit[448]{kbit/s}, stereo).
Subsequently, the scenes was processed with \emph{FFmpeg}\footnote{The FFmpeg project is available at \url{https://www.ffmpeg.org/}.} to re-encode with the desired \ac{QP} factor while leaving the audio unchanged.
...
For \E4{}, a speech telephony service and a \ac{VoD} service were implemented, which could be used by participants with their own personal computer while using their Internet access.
...
The service needed to be able to allow multiple two-party speech conversations at the same time while being able to insert packet-loss for individual conversations.
For this service, one server was installed in the data center of \emph{TU~Berlin} reachable by a public IPv4 address.
As operating system \emph{FreeBSD\,9.0} was selected, as it provides a built-in firewall with integrated traffic shaper called \emph{Dummynet}~\citep{rizzo_dummynet_1997}.
This traffic shaper allows to add network impairments such as packet-loss, delay, and jitter per individual connection, \ie, protocol, port, source address, and destination address.
...
First of all, participants connected using their own Internet access (minimal bandwidth: \unit[6]{MBit/s}).
...
However, for the media streams no counter measures were taken against packet-loss, \ie, \acs{PLC} nor \acs{FEC}. 
% are available for , as the used speech codec G.722 (Mode\,1) was used with neither \acs{PLC} nor \acs{FEC} on \acs{UDP} connections without retransmission.
Second, packet-loss was added by Dummynet additionally to packet-loss occurring due to connection issues resulting in potentially higher loss than desired.
...
This video player was bundled together with the preprocessed videos on a \acs{USB} flash drive.
Participants were instructed that for using the \ac{VoD} service the \acs{USB} flash drive must be inserted into the computer and remain connected while using this service.
...
This mobile phone provides a \unit[4.5]{inch} display with a resolution of \unit[480$x$800]{px}.
...
For this experiment, it was chosen not to provide any equipment for participants, and instead let participants use their own equipment.
...
This website used \textsc{\lowercase{HTML5}} multi-media features to download and playback the audio content.
Participants were only required to use \emph{Google Chrome}/\emph{Chromium} as web browser.
This limited the testing effort for the website.
As the buffering strategy for media content cannot be controlled by a website using standardized \textsc{\lowercase{HTML5}}, the preloading was initiated, when the website was loaded.
Here, a loading animation was presented for \unit[5]{s}.
This avoids issues due to initial buffering by providing a constant waiting time.

\chapter{Episodic and Multi-episodic Questions }\label{appendix:questions}
\begin{table}[h]
 \centering
 \caption{Questions for the episodic and multi\-/episodic judgments for all conducted experiments.}
 \label{tab:appendix:questions}
 \begin{tabulary}{\textwidth}{C|C|C}
   Experiment & Episodic & Multi-episodic\\
   \midrule
...
\begin{figure}[H]
	\centering
	<<plotE1, echo=F, fig.height=12>>=
		ggplot_timeseries_create(subset(timeseries, experiment=="E1")) + geom_boxplot(aes(factor(id), y=QU), na.rm=T) + facet_grid(condition ~ .) + xlab("Usage Episode")
	@
	\caption[One session (\E1{}): box plot of episodic judgments]{One session (\E1{}): box plot of episodic judgments.}
\end{figure}

\begin{figure}[H]
	\centering
	<<plotE2a, echo=F, fig.height=10.5>>=
		ggplot_timeseries_create(subset(timeseries, experiment=="E2a")) + geom_boxplot(aes(factor(id), y=QU), na.rm=T) + facet_grid(condition ~ .) + xlab("Usage Episode")
	@
	\caption[One session (\EIIa{}): box plot of episodic judgments]{One session (\EIIa{}): box plot of episodic judgments.}	
\end{figure}

\begin{figure}[H]
	\centering
	<<plotE2b, echo=F, fig.height=7.5>>=
		ggplot_timeseries_create(subset(timeseries, experiment=="E2b" & service != "bundle")) + geom_boxplot(aes(factor(id), y=QU), na.rm=T) + facet_grid(condition + service~ .) + xlab("Usage Episode")
	@
	\caption[One session (\EIIb{}): box plot of episodic judgments]{One session (\EIIb{}): box plot of episodic judgments.}
\end{figure}

\begin{figure}[H]
	\centering
	<<plotE3, echo=F, fig.height=3>>=
		ggplot_timeseries_create(subset(timeseries, experiment=="E3")) + geom_boxplot(aes(factor(id), y=QU), na.rm=T) + facet_grid(condition ~ .) + xlab("Usage Episode")
	@
	\caption[One session (\E3{}): box plot of episodic judgments]{One session (\E3{}): box plot of episodic judgments.}
\end{figure}

\begin{figure}[H]
	\centering
	<<plotE6, echo=F, fig.height=9>>=
		ggplot_timeseries_create(subset(timeseries, experiment=="E6a")) + geom_boxplot(aes(factor(id), y=QU), na.rm=T) + facet_grid(condition ~ .) + xlab("Usage Episode")
	@
...
<<REsetup, echo=F, cache=F, results='hide'>>=
	timeseries$condition = timeseries$condition_org
	suppressMessages(attach(timeseries))	
@
\begin{table}[H]
	\centering
	...
	\midrule
	\E1{} & \C1{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "1", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E1", "1", 6)} 		& -  \\
	\hline
	\E1{} & \CIIa{} & \Sexpr{mos_iqu_with_sd_by_condition("E1", "2a", 3)} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "2a", 6)} 	& - \\
	\hline
	\E1{} & \C3{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "3", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E1", "3", 6)} 		& -  \\
	\hline
	\E1{} & \C4{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "4", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E1", "4", 6)} 		& - \\ 
	\hline
	\E1{} & \CVb{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "5b", 3)} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "5b", 6)} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "5b", 9)}\\
	\hline	
	\E1{} & \C6{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "6", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E1", "6", 6)} 		& - \\
	\hline
	\E1{} & \C7{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E1", "7", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E1", "7", 6)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E1", "7", 9)}\\
	\hline
	\hline
	\EIIa{} & \C1{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "1", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "1", 6)} 		& - \\
	\hline
	\EIIa{} & \CIIa{} & \Sexpr{mos_iqu_with_sd_by_condition("E2a", "2a", 3)} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "2a", 6)} 	& - \\
	\hline
	\EIIa{} & \C3{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "3", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "3", 6)} 		& - \\
	\hline
	\EIIa{} & \C4{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "4", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "4", 6)} 		& - \\
	\hline
	\EIIa{} & \CVa{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "5a", 3)} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "5a", 6)} 	&  - \\
	\hline	
	\EIIa{} & \C6{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "6", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "6", 6)} 		& - \\
	\hline
	\EIIa{} & \C8{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "8", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E2a", "8", 6)} 		& - \\
	\hline
	\hline
	\EIIb{} (telephony) 	& telephony: \CVa{}; \ac{VoD}: - & \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a", 3, "telephony")} & \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a", 6, "telephony")} 		& - \\
	\hline
	\EIIb{} (telephony) 	& telephony: \CVa{}; \ac{VoD}: \C0{} & \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a0", 3, "telephony")} & \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a0", 6, "telephony")} 	& - \\
	\hline
	\EIIb{} (\ac{VoD})		& telephony: \CVa{}; \ac{VoD}: \C0{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a0", 3, "VoD")} 		& \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a0", 6, "VoD")} 		& - \\
	\hline
	\EIIb{} (telephony) 	& telephony: \CVa{}; \ac{VoD}: \C4{} & \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a4", 3, "telephony")} 	& \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a4", 6, "telephony")} 	& - \\
	\hline
	\EIIb{} (\ac{VoD}) 	& telephony: \CVa{}; \ac{VoD}: \C0{}	& \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a4", 3, "VoD")} 		& \Sexpr{mos_iqu_with_sd_by_condition("E2b", "5a4", 6, "VoD")} 		& - \\
	\hline
	\hline	
	\E3{} & \CIIa{} 	& \Sexpr{mos_iqu_with_sd_by_condition("E3", "2a", 3)} 		& \Sexpr{mos_iqu_with_sd_by_condition("E3", "2a", 6)}	& - \\
	\hline
	\E3{} & \CIIb{} & \Sexpr{mos_iqu_with_sd_by_condition("E3", "2b", 3)} 	& \Sexpr{mos_iqu_with_sd_by_condition("E3", "2b", 6)} 	& - \\
	\end{tabulary}
...
%\begin{table}[h]
%	\centering
%	\caption{The \acs{SCS} used in \E1, \E2, and \E3 were taken from \citet{itu-t_recommendation_p.805_subjective_2007} and out-dated information updated. The scenario of episode 4 was changed from a flight information and booking task to a change of booking task.}
%	\begin{tabular}{c|c|c}
%	Episode & Title & Page \\
%	\hline

...
Thus, this methodology allows to investigate the impact on multi-episodic perceived quality for defined condition.
...
This non-intrusive methodology is, however, in general limited by information that can be gathered about a user.
...
In fact, this applies also to the defined-use methodology as here users are instructed how to use a service.
...
A post-hoc test (pairwise Wilcoxon rank-sum test with Holms' correction) shows that all conditions except condition~2 and~3 are significant different ($p\leq0.137$).
...
The results show that defined-use methodology could be applied successful, because episodic judgments could be taken reliably even in a field experiment.
...
Although participants used the service in their home environment on their own, \ie, in an uncontrolled and to the researchers unknown setting, which is an additional source of noise, the episodic results show that perceived quality can be assessed successfully with this methodology.

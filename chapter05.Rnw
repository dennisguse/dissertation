...
To limit an influence of prior experiences and information about a service, it must be ensured that users have a similar knowledge about the service and similar prior exposure to the service.
...
The episodic judgments and multi\-/episodic judgments were both taken on the 7\=/point \ac{CoCR} scale \citep[][p.\,18]{itu-t_recommendation_p.832_subjective_2000}.\footnote{For a comparison between discrete 5\=/point \ac{ACR} scale and 7\=/point \ac{CoCR} scale see \citet{koster_comparison_2015}.}
...
In this experiment, performance was varied between usage episodes by limiting the maximum transmission bandwidth symmetrically, \ie, for both participants, for audio and video.
...
However, comparing the episodic judgments for \ac{HP} episodes between conditions\,1..4 show significant differences (\Sexpr{kruskal(QU~condition, "MOELLER", "HP", c("S1", "S2", "S3", "S5"))}).
...
Nevertheless, \citet{moller_single-call_2011} showed that the defined-use method can be applied in a field experiment.

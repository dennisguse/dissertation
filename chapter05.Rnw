...
The multi-episodic perceived quality is the result of an \emph{integrating process} combining prior experiences and perceived qualities with one service.
...
Multi-episodic perceived quality is thus a sequential process in which the order of usage episodes and their individual experiences affect the outcome.
...
The multi-episodic terminology is described by \citet[p.~8]{roto_user_2011}, but missed to put it into context with prior work and present their definitions.

\citet{karapanos_user_2009} investigated in an experiment with \unit[6]{subjects} how expectations and behavior towards a new smart phone change in a usage period of 4 weeks.
...
First work in direction on multi-episodic perceived quality was performed by \citet{duncanson_average_1969} for telecommunication services.
...
It could be shown that judgments for case a) yields a higher score than case b).
...
This is true for \ac{UX} as well as as for initial work on multi-episodic perceived quality of \citet{moller_single-call_2011}.
...
Perceived quality is influenced by prior knowledge about a service under consideration (\cf, \autoref{related:perceivedQuality}).
Prior knowledge includes knowledge about the specific service and also about the type of service in general.
...
This is for example expressed in the \emph{E-Model} by the \emph{advantage factor} \citep{itu-t_g.107:_2014} that expresses a favorable judgment due to expected difficulty of service provision, \eg, use of a mobile phone in a remote area.
...
This includes human factors like attention, but also needs and tasks.
...
For example high delay in a telephone conversation with rare turn-taking will be judged better than a conversation requiring more turn-taking \citep[\cf,][]{egger_it_2010, schoenenberg_quality_2015}.
...
A higher importance of a usage episode might increase the likelihood to memorize and recall information about this episode.
Thus this episode might have a higher impact on the multi-episodic judgment.
...
This includes how often a user is exposed to the service's performance and thus acquires a new perceived quality.
...
\paragraph{Assessment Methodologies} %TODO Different structure to avoid looking like a subsection without number
...
Often, information about a user's usage pattern, service performance, tasks, goals, expectations, prior knowledge, and also personality cannot be acquired.
This lack of information can be overcome to a certain degree by sampling a large amount of users.
...
Allowing users to decide when and under which conditions a service is used, enables to investigate usage under \emph{real conditions}\footnote{\emph{Real conditions} means that the object under study is observed while interacting with his \emph{real} environment. In fact, the term "real" does not describe precisely the actual condition, but is an imprecise umbrella term.}, \ie, how a user would behave in his current situation.
...
Creating a new service does not prevent a user from having prior knowledge and also expectations about such a type of service.
However it should limit the influence of prior experiences with other, similar services.
Enforcing defined-use limits variations in usage behavior and task solving behavior, but enables to expose multiple participants to similar conditions.
Thus, this method allows to investigate the impact on multi-episodic perceived quality for defined condition.
...
This non-intrusive method is, however, in general limited by information that can be gathered about a user.
...
In fact, this applies also to the defined-use method as here users are instructed how to use a service.
For this method, a service must be available that is able to provide performance per usage episode in a precise and reliable manner.
...
In addition to technological challenges in deployment of such a service, a user needs to solve artificial tasks at a specific point in time.
This is not an issue, if multi-episodic perceived quality is studied in one session, \eg, several consecutive usage episodes.
Investigating multi-episodic perceived quality over larger time-spans covering several days, weeks, or months, however, increases the required effort for a user to stick to the defined usage pattern.
...
The first work on multi-episodic perceived quality while applying the defined-use method was performed by \citet{moller_single-call_2011}.
...
For each of those \unit[24]{calls} a \ac{SCT} \citep[ITU-T~P.805][]{itu-t_p.805:_2007} needed to be solved.
...
\acp{SCT} try to enforce a conversation flow, which is however influenced by the usage behavior of the two parties, and thus might affect the individual duration of usage episodes as reported by \citet{moller_single-call_2011}.

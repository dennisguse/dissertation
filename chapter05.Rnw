...
Such a set this is denoted as \emph{multi\-/episodic condition}.
...
The service needed to be used twice per day, \ie, first call between \unit[6]{h} and \unit[15]{h} and the second call between \unit[15]{h} and midnight.
...
However, comparing the episodic judgments for \ac{HP} episodes between the conditions~1..4 show significant differences (\Sexpr{kruskal(QU~condition, "MOELLER", "HP", c("S1", "S2", "S3", "S5"))}).
...
However, the judgment after the 2nd~day is not significantly different from the judgment after the 12th~day as well as the judgment after the 7th~day is not significantly different from the judgment after the 12th~day (each \Sexpr{wilcox.pairwise.print(IQU~id, "MOELLER", c("LP", "HP"), "S2", c(4, 14, 24))[2,2]}).

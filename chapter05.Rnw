...
It is for example not clear how Skype distributed the defined transmission bandwidth between the audio and video channel, and if a part of the transmission bandwidth was used for \ac{FEC} or other mechanism to improve robustness against packet-loss.

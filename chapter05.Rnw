...
@
\chapter{Multi-episodic Perceived Quality}\label{chap:state-of-the-art}
...
\section{Initial work with Defined-use \citep{moller_single-call_2011}}
...
Three performance levels were applied: \unit[500]{kbit/s} (\ac{HP}), \unit[150]{kbit/s} (\ac{MP}), \unit[32]{kbit/s} (\ac{LP}).
...
\begin{table}[h]
	...
	\midrule
	1 & \ac{HP} & \textbf{\ac{LP}} & \ac{HP} & \ac{HP} & \textbf{\ac{LP}} & \ac{HP} & \ac{HP} \\
	\hline
	2 & \ac{HP} & \textbf{\ac{LP}} & \textbf{\ac{LP}} & \ac{HP} & \textbf{\ac{LP}} & \textbf{\ac{LP}} & \ac{HP} \\
	\hline
	3 & \ac{HP} & \textbf{\ac{LP}} & \ac{HP} & \ac{HP} & \ac{HP} & \ac{HP} & \ac{HP} \\
	\hline
	4 & \textit{\ac{MP}} & \textbf{\ac{LP}} & \textit{\ac{MP}} & \textit{\ac{MP}} & \textbf{\ac{LP}} & \textit{\ac{MP}} & \textit{\ac{MP}} \\
	\hline
	5 & \ac{HP} & \ac{HP} & \ac{HP} & \ac{HP} & \ac{HP} & \ac{HP} & \ac{HP} \\
	\end{tabular}
...
The results show that episodic quality judgments reflect the desired performance and thus that the system functioned as desired and those quality judgments can be reliably taken in a field studies.



With regard to episodic judgments sequential effects of episodic judgments are reported by \citet{moller_single-call_2011}.
All five profiles have the tendency to slightly increase from the first to last usage episode.
An average increase over all conditions of \unit[0.31]{points} on the 7-point \ac{CCR} scale is reported.
...
The following detailed analysis of multi-episodic perceived quality judgments is undertaken by the author of this thesis using the data provided by \citet{moller_single-call_2011}.
\autoref{tab:chap05:multi} shows \ac{MOS} of those judgments for all conditions.
...
\begin{table}[h]
	...
	Condition 	& Participants 	& \multicolumn{3}{c}{Multi-episodic judgment at day} \\
				&				& \ac{HP}	& \ac{MP} & \ac{LP}\\
	\midrule
	1 			& 16 			& \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "HP", "S1")} & - & \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "LP", "S1")} \\
	\hline
	2 			& 10 			& \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "HP", "S2")} & - & \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "LP", "S2")} \\
	\hline
	3 			& 10 			& \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "HP", "S3")} & - & \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "LP", "S3")} \\
	\hline
	4 			& 10 			& -	& \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "MP", "S4")} & \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "LP", "S4")} \\
	\hline
	5 			& 10 			& \Sexpr{mos_qu_with_sd_by_condition("MOELLER", "HP", "S5")} & - & - \\
	\end{tabular}
...
\begin{table}[h]
	...
	Condition 	& Participants 	& \multicolumn{3}{c}{Multi-episodic judgment at day} \\
				&				& 2	& 7 & 12 \\
	\midrule
	1 			& 16 			& \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S1", 4)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S1", 14)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S1", 24)} \\
	\hline
	2 			& 10 			& \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S2", 4)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S2", 14)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S2", 24)} \\
	\hline
	3 			& 10 			& \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S3", 4)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S3", 14)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S3", 24)} \\
	\hline
	4 			& 10 			& \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S4", 4)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S4", 14)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S4", 24)} \\
	\hline
	5 			& 10 			& \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S5", 4)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S5", 14)} & \Sexpr{mos_iqu_with_sd_by_condition("MOELLER", "S5", 24)} \\
	\end{tabular}
	\caption{Multi-episodic perceived quality judgments as found in the experiment of \cite{moller_single-call_2011}. Multi-episodic judgments reported as \ac{MOS} with standard deviation in brackets.}
	\label{tab:chap05:multi}
\end{table}
...
It is for example not clear how Skype distributed transmission bandwidth between the outgoing audio and video channel \citep[\cf][]{moller_single-call_2011}, and if a part of the transmission bandwidth was used for \ac{FEC}.

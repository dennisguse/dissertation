...
Also, a user's behavior towards a service might change due to perceived quality affecting usage frequency or task-solving strategies, or even lead to abandoning the service completely.
...
Investigations of multi\-/episodic perceived quality can therefore only be undertaken in experiments adhering to a between-subject design.
...
Multi-episodic evaluation is an important aspect of \ac{UX}, as user behavior towards technology changes as a user learns how to use the technology and which tasks are well-suited to its use.
...
This also includes recalled changes in usage behavior.
Using this method, an experiment was conducted evaluating the changes of satisfaction and user behavior with a new smart phone over a usage period of one year.
...
\paragraph*{Aspects}
Perceived quality for short stimuli as well as complete usage episodes is investigated by exposing multiple participants to same stimulus/condition and assessing the resulting perceived quality. %\citep[][p.\,11]{blauert_spatial_1996}.
...
For example, an interviewee in a telephone job interview is likely to recall this specific usage episode and describe his perceived quality in difference to a common call with a friend.
Therefore, a task, as well as its task-solving strategies and importance, should be kept constant to avoid an undesired impact, \ie, not explainable, on multi\-/episodic perceived quality.
...
Besides the usage pattern, the experienced performance  seems to be greater importance.
...
In this setting, each user can decide individually when, how, how often, and for which tasks, and how to use this service, \ie, the individual user behavior is not limited.
...
Allowing users to decide when and how a service is used enables the investigation of usage in an ecologically valid situation, \ie, how  users would behave in a current situation.
This method offers ecological validity, but limits reproducibility.

\subparagraph*{Defined-use}\label{method:definedUse}
Limiting the user's freedom by defining when and how to interact with a service allows to derivation of  a \ac{MOS}.
...
In addition, the performance of a service can hardly be controlled and the desired degradations might not occur reliably enough.
...
In addition to limited ecological validity, this method requires that a service is available that can provide the desired performance in a precise and reliable manner.
...
Longer time-spans also increase the likelihood that a defined usage pattern cannot be fulfilled by an individual participant, because each participant must integrate the usage pattern into his daily life.
...
\citet{moller_single-call_2011}\footnote{It must be noted that \citet{moller_single-call_2011} uses the term \emph{service quality} as synonym to multi\-/episodic perceived quality, rather than service quality in terms of \citet{parasuraman_conceptual_1985} (\cf, \autoref{chap:03}).} followed the premise that the service works in general, \ie, it provides the best achievable performance, but sometimes usage episodes are only presented with reduced performance.
It was necessary to use the service twice a day, with the first call between \unit[6]{h} and \unit[15]{h} and the second call between \unit[15]{h} and midnight.
For each of these \unit[24]{calls}, a \ac{SCS} needed to be solved \citep{itu-t_recommendation_p.805_subjective_2007}.
...
The episodic judgments and multi\-/episodic judgments were both taken on a 7\=/point \ac{CoCR} scale \citep[][p.\,18]{itu-t_recommendation_p.832_subjective_2000}.\footnote{For a comparison between discrete 5\=/point \ac{ACR} scale and 7\=/point \ac{CoCR} scale see \citet{koster_comparison_2015}.}
...
Here, the condition defines the time for each usage episode, task, and also performance level.
In each condition, usage episodes were presented on one or two performance levels.
...
In this experiment, participants were equipped with a headset and a webcam to avoid any impact of varying terminal equipment~\citep[][]{moller_single-call_2011}.
...
It must be noted that although \ac{HP} and \ac{LP} are different, \ac{LP} was still judged approximately as \emph{fair}.
...
The results show that the defined-use method could be applied successfully, since episodic judgments could be taken reliably even in a field experiment.
...
In terms of multi\-/episodic judgments, the result of this experiment are limited.
...
For condition\,4, a post-hoc test (pairwise Wilcoxon rank-sum test with Holms' correction) shows that the multi\-/episodic judgment on the 2nd\,day is significantly different from the judgment after the 7th\,day (\Sexpr{wilcox.pairwise.print(IQU~id, "MOELLER", c("LP", "HP"), "S2", c(4, 14, 24))[1,1]}).
However, the judgment after the 2nd\,day is not significantly different from the judgment after the 12th\,day, in addition to the judgment after the 7th\,day is not significantly different from the 12th\,day (each \Sexpr{wilcox.pairwise.print(IQU~id, "MOELLER", c("LP", "HP"), "S2", c(4, 14, 24))[2,2]}).
In fact, an absolute comparison with regard to multi\-/episodic judgments between the five conditions conducted must be omitted due to the observed differences for episodic judgments between conditions, \ie, an artifact that might be attributed to the between-subject design.
...
\subparagraph{Discussion}
\citet{moller_single-call_2011} successfully applied the defined-use method for the assessment of multi\-/episodic perceived quality in a field experiment with a usage period of \unit[12]{days}.
...
However, the multi\-/episodic perceived quality judgments only provide limited insight, since the differences between conditions are rather small.

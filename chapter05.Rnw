...
The definition of \emph{usage episode} in the context of multi\-/episodic perceived quality is derived from concepts of episodic memory (\cf, \autoref{chap:03}), focusing on usage of telecommunication systems.
...
to understand the user's perspective and provide a better service.
...
\citet{karapanos_user_2009} investigated in an experiment, how the expectations and behavior towards a new smart phone change in a usage period of four weeks. % with \unit[6]{subjects}
This is extended by \citet{kujala_ux_2011}.
Here, a participant evaluates his experiences with a product or service in retrospective.
...
With this method an experiment was conducted evaluating the changes of satisfaction and user behavior with a new smart phone over a usage period of one year.
...
In the beginning interactions are more playful and exploratory, but over time get more task-oriented and productive.

Although multi\-/episodic integration is not a key aspect for \ac{UX}, it is an important aspect especially regarding the adaption of new products and new services.
...
This suggests that the formation process does not equally weight all prior experiences with said service, but rather focuses experiences with low performance.
...
With regard to multi\-/episodic perceived quality and perceived quality over time in general, a \ac{MOS} can only be calculated with judgments that result from the identical or very similar conditions.
In fact, this implicitly assumes that effects/biases are similar for different participants with regard to the formation process of multi\-/perceived quality, \ie, the same effects with a similar characteristic occur for each participant.
...
For the investigation of multi\-/episodic perceived quality, this must be taken into account, as a potential factor of noise.
...
In such a case, latter episodes will require less time and thus might affect the multi\-/episodic quality formation process.
...
For example an interviewee in a telephone job interview is likely to recall this specific usage episode and describe his perceived quality compared to a common call with a friend.
Therefore, task, solving strategies, and its importance should be kept constant to avoid an undesired impact, \ie, not explainable, on multi\-/episodic perceived quality.
In addition, tasks should be selected that yield a comparable task solving duration.

%\subparagraph*{Usage Pattern}
Beside prior knowledge and tasks, also the actual \emph{usage pattern} might affect multi\-/episodic judgments.
...
However, so far the potential impact of those aspects on multi\-/episodic judgments is not yet known.
...
To limit an influence of prior experiences and information about a service, it must be ensured that users have a similar knowledge of the service and similar prior exposure to the service.
This can be achieved by a careful selection of users or by creating a \emph{new service}.
In fact, a new service does only prevent the existence of prior experiences with this service, but cannot avoid the impact of prior knowledge for example about typical degradations for such a service type in general.
This is here denoted as defined-use method.
Here, the usage pattern, tasks, and performance are defined.
This is denoted as \emph{multi\-/episodic condition}.
Exposing multiple participants to the same multi\-/episodic condition allows to derive a \ac{MOS}.
The impact of performance on multi\-/episodic perceived quality can then be investigated by varying only the performance between otherwise identical conditions.
...
After the 2nd\,defined call on the 2nd, 7th, and 12th day the perceived quality of all experienced usage episodes \emph{so far} was assessed (\emph{multi\-/episodic judgment}).
The episodic judgments and multi\-/episodic judgments were both taken on a 7\=/point \ac{CoCR} scale \citep[][p.\,18]{itu-t_recommendation_p.832_subjective_2000}.\footnote{For a comparison between discrete 5\=/point \ac{ACR} scale and 7\=/point \ac{CoCR} scale see \citet{koster_comparison_2015}.} The scale is shown in \autoref{img:chap05:quality-scale}.
...
The observed difference for episodic judgments of \ac{HP} are relatively small and are likely an artifact of the applied between-subject design.
...
In fact, \ac{MP} judgments are only slightly lower than \ac{HP} judgments for condition~1..4.
As \ac{MP} and \ac{HP} were not presented together, this indicates that either the scale is used differently between conditions, or that both performance levels were judged similarly.
...
In addition, it is reported that the judgments of \ac{HP} episodes which follow \ac{LP} episodes, seem to be negatively affected by the preceding \ac{LP} episodes.
...
A significant difference between multi\-/episodic judgments per condition is only found for condition\,4 (\Sexpr{kruskal(IQU~id, "MOELLER", c("LP", "HP"), "S2", c(4, 14, 24))}).
...
However, the judgment after the 2nd~day is not significantly different to the judgment after the 12th~day as well as the judgment after the 7th\,day is not significantly different to the 12th\,day (each \Sexpr{wilcox.pairwise.print(IQU~id, "MOELLER", c("LP", "HP"), "S2", c(4, 14, 24))[2,2]}).
...
Beside this, an issue with this experiment is that no useful details about the technical parameters were provided.
...
Some participants might have used the service more often than others.
This might have affected their episodic and multi\-/episodic quality judgments.
...
Nevertheless, \citet{moller_single-call_2011} showed that multi\-/episodic perceived quality can be assessed with the defined-use method in a field experiment.

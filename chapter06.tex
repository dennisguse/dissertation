...
Then, I describe the hypotheses I am going to investigate about the formation process of multi\-/episodic perceived quality.
...
This might lead to effects that the highest performance is judged to be better after reduced performance episodes have been experienced, because the latter might lead to an adjustment of the internal reference.
...
It is assumed that differences in performance level between multi\-/episodic conditions, if those are different enough, manifest as consistent differences in the multi\-/episodic judgments.
...
\ac{HP} and \ac{LP} were presented in all conditions per experiment, so that the judgments of the conditions are directly comparable and are not affected by differences in worst and best performance levels.
This is useful in the investigation of the impact of the necessary between-subject design.
...
However, the impact on multi\-/episodic judgments due to \ac{LP} usage episodes is very limited (\cf, \autoref{prior:moeller}).
...
In fact, limiting the usage period to such a short time-frame reduces the required effort and thus allows the investigation of a higher number of conditions in detail.
...
Based upon this information, the callee selects an appropriate predefined option or information and presents this to the caller.
...
In fact, a passive observer is not even forced to follow a conversation, as he does not have to react to the content.
...
For such a service, a user can select from the available content that which is currently desired; this is then transmitted to him.
...
First, such a service can be set up in a non\-/interactive way, thus avoiding the impact of varying user behavior.
...
The experimental investigation of these hypotheses will be used for the implementation of a prediction model for multi\-/episodic perceived quality in \autoref{chap:modeling}.
...
The more \ac{LP} episode(s) are presented, the higher the expected reduction of multi\-/episodic judgments.
Here, a lower boundary is expected to be set by the episodic judgments for \ac{LP} episodes.

Based upon this hypothesis, the impact of \ac{LP} episodes can also be quantified.
...
If a peak effect exists, then the usage episode(s) presented with the worst performance level should have a higher impact on the multi\-/episodic judgment than usage episodes which achieve a better perceived quality.
...
Multi-episodic perceived quality can only be assessed in retrospect by evaluating prior, recallable experiences of a service and create the judgment.
...
With regard to modeling, most important seems to be \autoref{hypo:number}, because this hypothesis should result in a large effect, while the other hypotheses provide knowledge about edge of the formation process of multi-episodic perceived quality.

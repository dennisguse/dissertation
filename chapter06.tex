...
This allows a participant to familiarize with the highest performance level and his resulting perceived quality.
...
This might lead to effects that highest performance is judged better after reduced performance episodes have been experienced, because the latter might lead to an adjustment of the internal reference.
...
\ac{HP} and \ac{LP} were presented in all conditions per experiment, so that the judgments of the conditions are directly comparable and are not affected by differences in worst and best performance.
...
Anyhow, using recordings of a complete two\=/party conversation is here considered to represent a usage episode if it contains a meaningful conversation.
...
If the desired degradations do not affect the behavior of caller and callee, the degradations can even be inserted by post\=/processing the recordings.
...
For such a service, a user can select from the available content the currently desired item, which is then transmitted to him.
...
First, such a service can be set up in a non-interactive way and thus avoids varying user behavior.
...
Here, if the number of performance changes between episodes affects multi\-/episodic judgments.
...
In fact, keeping the number of \ac{HP} and \ac{LP} episodes constant, \ac{LP} episodes must be separated by \ac{HP} episode(s) in non-consecutive cases.
...
This allows to determine the impact of \ac{MP} episodes on multi\-/episodic judgments compared to \ac{LP} episode(s) and thus investigate the existence of a peak effect.

...
\citet{moller_single-call_2011} evaluated the precision of this approach with their \unit[14]{day} experiment (\cf, \autoref{prior:moeller}).
...
However, the results are limited due to the limited impact on multi\-/episodic judgments of the conditions, \ie, presentation of \ac{LP} episodes.
...
Following, model types are presented that seem suited for predicting multi\-/episodic judgments.
...
In difference to \cite{moller_single-call_2011} the goal here is to predict the multi\-/episodic \ac{MOS} based upon the episodic \ac{MOS} rather than the judgments of individual participants.
The latter might desirable, but the conducted experiments did not collect sufficient data to be able to explain individual differences.
In fact, the defined\-/use method was applied, so a \ac{MOS} could be derived.
...
The multi\-/episodic judgment after the 6th usage episode showed a negative effect, but the judgments after the 3rd and 9th usage episode were not significantly different.
...
In fact, a static window is a rather unlikely case, because the importance of episodic judgments is considered binary.
...
However, the actual window is increased by $\mathit{2*w}$, so the very first episodic judgment can be considered with a maximum weight of $\geq 0.5$.\footnote{Please note that normalization due to the weight function is handled by the weighted average.}
...
In the following, a \emph{model} denotes the selected weight function with the defined parameter~$\mathit{w}$.
...
Furthermore, even the \ac{RMSD} behaves similarly for individual conditions.
...
For both experiments it is notable that the similar values for the $\mathit{w}$ were found for each of the two weight functions, although the usage situation is different.
With regard to prediction accuracy LI is preferable over~WI.
Besides, LI seems to be more robust against selected parameter.
For both experiments a minimal \ac{RMSD} could be achieved for $\mathit{w}=2$ for LI.
For predicting the multi\-/episodic judgments after the 9th usage episode $\mathit{w}=3$ (LI) performs best (only \E1{}). 
...
Here, increasing $\mathit{w}$, improves the prediction accuracy.
...
In both cases the multi-episodic judgment remains above the episodic judgment for \ac{LP} episodes.
\C5{} and \C6{} only differ in the episodic performance of the 4th episode/day.
For \C5{} this episode is presented in \ac{HP} while \C6{} presented this episode/day in \ac{LP}.
As both conditions are not judged different, this suggests that the performance level of the 4th episode/day does not seem to affect the formation process of the multi-episodic judgment.
...
It must be noted that the optimal value of $\mathit{w}$ in the case of \C5{} is identical to the smallest near optimum values for $\mathit{w}$ for LI and WI.
Thus, the overall prediction performance for both weighting functions can be improved slightly if $\mathit{w}$ can be reduced to the smallest near optimum value for \C6{}.
Although the underlying reason for the observed saturation effect cannot be deduced from the experiments alone, this can be achieved by \emph{modifying} the episodic judgment(s) of the 4th episode/day for \C6{}, so it resembles \C5{} with regard to episodic judgments, \ie, replacing it by the average of episodic judgment of \ac{HP} episodes.
...
For \E1{} and \EIIa{}, this is shown in \autoref{fig:pred:SAT:E1}1} and \autoref{fig:pred:SAT:E2a}, respectively.
...
In fact, it must be noted that the weighted average using rather simple weight functions with one degree of freedom enables a decent prediction accuracy.
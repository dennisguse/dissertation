...
Following, model types are presented that seem suited for the prediction.
The prediction accuracy for those model types is evaluated using only \E1{}, \EIIa{}, and \E6{}.
...
In \EIIa{} and \E6{} also the impact of non\-/consecutive \ac{LP} presentation was investigated (\autoref{hypo:consecutive}).
...
A peak effect is not considered for modeling, because such an effect could not be observed in \E1{} (\autoref{hypo:strength}).
...
Here, nine episodes were presented while non-\ac{HP} episodes were presented as the 5th and the \unit[6th]{episode}.
...
For one session also a duration neglect was investigated in \E3{} (\autoref{hypo:duration}). 
...
Thus, LI is preferable, as it has a higher robustness for selecting $\mathit{w}$.
...
The omission of \C4{} shows similar results to~\E1{}, \ie, the prediction accuracy is not affected by the selection of $\mathit{w}$.
In fact, \ac{RMSD} reaches a similar value than for~\E1{} for all $\mathit{w}$ ($> 0.1$).
...
With regard to the applicability of a weighted average model this is problematic, as a model needs to produce a similar prediction based upon different input, \ie, one \ac{HP} episode/day and two \ac{LP} episodes/days (\C5{}) versus three \ac{LP} episodes/days (\C6{}).
...
For all conditions, this leads to a reduction of the \ac{RMSD} (\E1{}: 0.02, \EIIa{}: 0.03, and \E6{}: 0.08).
...
For both models, the highest prediction accuracy is achieved for the same $\mathit{w}$ for \E1{} and \EIIa{}, \ie, LI:~$\mathit{w}=2$ and WI:~$\mathit{w}=4$.

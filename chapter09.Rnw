...
The weight for an episode is denoted as~$\mathit{a_i}$.
...
However, this is a strong deviation due to~\C4{} alone.
...
Furthermore, the minimal \ac{RMSD} is reached earlier, but with increasing $\mathit{w}$ the \ac{RMSD} is slightly worse for \C6{}\,(adjusted) than \C6{}.
\begin{figure}[h]
	\centering

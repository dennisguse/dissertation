...
Here, the impact of \ac{LP} episode(s) on the following multi\-/episodic judgment decreases the more \ac{HP} episodes are presented afterwards.
...
Here, multi\-/episodic judgments remained on the same level independent if two or three \ac{LP} episodes/days were presented, \ie, \C5{}\footnote{In the following, \C5{} refers to \CVb{} in case of \E1{} and to \CVa{} in case of \EIIa{}.
} and \C6{} were not judged differently.
In both cases, the multi\-/episodic judgments remained above the episodic judgments for \ac{LP} episodes (approx.~\unit[1]{pt}).
...
In the following, the prediction accuracy of the two weight function for \C5{}, \C6{}, and \C6{}~(adjusted) is presented.
...
In fact, if \ac{LP} usages episodes were presented, both outperform the unweighted average of all prior episodic judgments, \ie, a lower $\mathit{w}$ achieves a better prediction accuracy.
...
In fact, it must be noted that the weighted average with the rather simple weight functions (one degree of freedom) enables a decent prediction accuracy for one session as well as a usage period of 6~days.

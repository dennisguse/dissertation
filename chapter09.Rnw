...
For example, \C3{} is far off for $w=1$ and improves until~3 while \C4{} is best predicted with~5.
Linear outperforms window in prediction accuracy and is more consistent.
The best accuracy is observed by setting $w$~to~2.
All conditions except \C4{} and \C6{} yield here the minimal \ac{RMSD}.
In fact, \C4{} and \C6{} reach their minimum at~3, but accuracy is already sufficient at $w=2$.
It must be noted that \ac{RMSD} is in the best case close to \Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E1" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 1)} (linear, $w=2$).
...
In fact, the latter provides provides near perfect accuracy for $w=4$, but is otherwise far off.
A linear weight performs very well for \CVb{} with $w \geq 3$ and very similar for \C7{} over the whole range.
...
Here, the prediction accuracy increases with an increasing $w$.
However, this is due to strong deviation of \C3{} alone. 
Omitting \C3{} shows similar results to \E1{}, \ie, prediction accuracy is not affected by $w$.
...
For window a minimum is reached at 3~to~4 and for linear at $w=2$.
...
For both experiments a minimal \ac{RMSD} could be achieved for $w=2$.
The multi-episodic judgments after the 9th usage episode is optimal for $w=3$ (only \E1{}). 
However, as only two conditions were adjusting $w$ seems improper.
...
Here, increasing $w$ results in an increase in prediction accuracy.
This is more prevalent for window than for linear.
Window achieves its minimal \ac{RMSD} with $w=6$ while linear provides only a slight decrease after $w=3$.
...
While linear reaches a minimum at $w=4$, window achieves its minimum not until $w=8$.
...
A linear weighting performs best for $w=4$.
...
Furthermore, it is notable that the minimal \ac{RMSD} is reached earlier, but that increasing the $w$ further is worse for \C6{}~(adjusted) than \C6{}.
\begin{figure}[h]
	\centering
...
This allows to reduce $w$.
In fact, for all three experiments $w$ is reduced to the optimal found solution for all conditions (see \autoref{results:prediction}).
...
Here, both model types show the highest prediction accuracy for the same $w$.

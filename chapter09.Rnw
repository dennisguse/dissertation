...
However, the actual window is increased by $\mathit{2*w}$, so the very first usage episode can be considered with a maximum weight  $\geq 0.5$.\footnote{
Please note that normalization due to the weighting function is handled by the weighting average.}

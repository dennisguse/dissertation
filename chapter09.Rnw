...
However, no impact of the presentation of a second service could be observed independent if the service was presented wit or without \ac{LP} episodes.

\section{Types of Model}
%For the prediction of retrospective quality judgment based upon momentary judgments the average has been found useful (\cf, \autoref{chap:03}).

For the implementation of a prediction model the weighted average is used (\autoref{eq:average}).
A weighted average 

\begin{equation}\label{eq:average}
\hat{m_n}=\frac{\sum\limits_{i=1}^{n}a_i*e_i}{\sum\limits_{i=1}^{n}a_i}
\end{equation}
...
\section{Evaluation: One Session} 


\section{Evaluation: Multiple Days}

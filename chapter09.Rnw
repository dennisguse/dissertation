...
The model\footnote{The term \emph{model} is used in the following in terms of curve fitting. A model is here a mathematical function, which might be parameterizable, and maps an input space to an output space.} proposed here is to average all \emph{prior} episodic judgments to predict a multi\-/episodic judgment.
...
This chapter starts with an overview of the effects observed in the here presented experiments that seem relevant for prediction of multi\-/episodic judgments.
...
The here presented modeling is necessarily limited to the development of best fitting predictor(s) as no data sets for cross\-/validation are available.
Furthermore, he conducted experiments differ slightly in the experimental design and observed effects.
...
The multi\-/episodic judgment after the 6th~episode showed a negative effect, but the judgments after the 3rd and 9th~episode were not significantly different.
...
In this thesis, weighted average models are proposed to increase prediction accuracy.
These allow to assign an individual weight to each episodic judgment and thus model the impact on a multi\-/episodic judgment that is to be predicted.
...
Here, WI performs better while increasing $\mathit{w}$ to 4 (\Sexpr{round(subset(data_mos_nonadjusted, data_mos_nonadjusted[["experiment"]] == "E1" & data_mos_nonadjusted[["id"]] == 6 & data_mos_nonadjusted[["model"]] == "weight_window_create" & data_mos_nonadjusted[["condition"]] == "All" & data_mos_nonadjusted["parameter"] == 4)["RMSD"], 2)}).
...
\EIIa{} complements \E1{} with a passive usage situation while it shares the same performance levels.
...
For WI, the minimal \ac{RMSD} is reached at $\mathit{w}=4$ (\Sexpr{round(min(subset(data_mos_nonadjusted, data_mos_nonadjusted[["experiment"]] == "E2a" & data_mos_nonadjusted[["id"]] == 6 & data_mos_nonadjusted[["model"]] == "weight_window_create" & data_mos_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)}) while $\mathit{w}=3$ (\Sexpr{round(subset(data_mos_nonadjusted, data_mos_nonadjusted[["experiment"]] == "E2a" & data_mos_nonadjusted[["id"]] == 6 & data_mos_nonadjusted[["model"]] == "weight_window_create" & data_mos_nonadjusted[["condition"]] == "All" & data_mos_nonadjusted["parameter"] == 3)["RMSD"], 2)}) is rather close.
...
LI provides only a marginal decrease after $\mathit{w}=3$.
...
With regard to the prediction the multi\-/episodic judgment of the 6th~day, both weight functions perform differently.
...
In addition, LI achieves a minimal \ac{RMSD} of \Sexpr{round(min(subset(data_mos_nonadjusted, data_mos_nonadjusted[["experiment"]] == "E6a" & data_mos_nonadjusted[["id"]] == 14 & data_mos_nonadjusted[["model"]] == "weight_linear_create" & data_mos_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)}, and WI only achieves a minimal \ac{RMSD} of
...
Here, multi\-/episodic judgments remained on the same level independent if two or three \ac{LP} episodes/days were presented, \ie, \C5{} and \C6{} were not judged differently.
...
As both conditions are not judged differently, this suggests that the difference in performance level of the 4th~episode/day does not seem to affect the formation process of the multi\-/episodic judgment.
In the following, \C5{} refers to \C5b{} in case of \E1{} and to \C5a{} in case of \EIIa{}.
...
For \E1{} and \EIIa{}, $\mathit{w}=3$~(LI) and $\mathit{w}=4$~(WI) are required for \C6{}, whereas \C5{} achieves its lowest prediction error already at $\mathit{w}=2$~(LI) and $\mathit{w}=3$~(WI).
...
ggplot_model_create(subset(data_mos_adjusted_only, data_mos_adjusted_only$experiment == "E6a" & data_mos_adjusted_only$id == 14), 2.0) + 
	geom_line(aes(x=parameter, y=RMSD, linetype=condition, size=condition, colour=condition)) + facet_grid(. ~ model_FACET) + 
	scale_linetype_manual(name="Condition", values=c("solid", "twodash", "solid"), labels=c("C5", "C6 (adjusted)", "C6")) +
	scale_color_manual(name="Condition",values=c("#00BFC4", "#7CAE00", "#7CAE00"), labels=c("C5", "C6 (adjusted)", "C6")) +
	scale_size_manual(name="Condition", values=c(0.5, 0.5, 0.5), labels=c("C5", "C6 (adjusted)", "C6"))
@
...
It could be shown that a weighted average using either a window function or a linear function enables to predict multi\-/episodic judgments.
...
Nevertheless, the conducted modeling is inherently limited to the data of the conducted experiments, and models could not be verified as no data sets that could be used for cross\-/validation were available.
...
To avoid overfitting $w$ is limited to $0 < w \leg 2*n$.
...
A modeling a peak-effect is omitted here, because such an effect could not be observed in the conducted experiments.
Even, if in \E1{} \C7{} a peak-effect occurred, the influence on the final judgment seems to be rather small.

\section{Prediction Accuracy}
In the following, the prediction accuracy is evaluated using two metrics.
Here, the \ac{RMSD} (\autoref{eq:rmsd}) and the Pearson correlation is used.
...
In a perfect case an \ac{RMSD} of 0 and correlation coefficient of 1 should reached.
In fact, a parameter set is preferred that suits most conditions rather than only explain individual conditions.

\section{Evaluation}


\subsection{One Session}

\subsection{Multiple Days}

...
data$model_FACET = factor(data$model == "weight_linear_create", labels=c("Window (WW)", "Linear (LI)")) #label and order; FALSE will be first; then attach labels for FACET
...
data_nonadjusted_avg$model_FACET = factor(data_nonadjusted_avg$model == "weight_linear_create", labels=c("Window (WW)", "Linear (LI)")) #label and order; FALSE will be first; then attach labels for FACET
...
data_adjusted_only=subset(data, data$condition %in% c("5a_average", "5b_average", "6_average", "6_adjusted_average"))
data_adjusted_only$condition = factor(data_adjusted_only$condition, labels=c("C5a", "C5b", "C6 (adjusted)", "C6"))

data_adjusted=subset(data,  grepl("average", data$condition) & data$condition != "6_average" & data$experiment %in% c("E1", "E2a", "E6a"))

data_adjusted_avg = aggregate(data_adjusted$RMSD, by=list(experiment=data_adjusted$experiment, model=data_adjusted$model, parameter=data_adjusted$parameter, id=data_adjusted$id), FUN=mean)
names(data_adjusted_avg)[5] = "RMSD"
data_adjusted_avg$condition = "EXPERIMENT_average"
data_adjusted_avg$PEARSON = NA
data_adjusted_avg$model_FACET = factor(data_adjusted_avg$model == "weight_linear_create", labels=c("Window (WW)", "Linear (LI)")) #label and order; FALSE will be first; then attach labels for FACET
data_adjusted = rbind(data_adjusted, data_adjusted_avg)

data_adjusted$condition = factor(data_adjusted$condition, labels=c("C1", "C2a", "C3", "C4", "C5a", "C5b", "C6", "C7", "C8", "All"))
@
...
Those experiments yielded a large data set in itself consistent data set.\footnote{An approach towards modeling based upon the experiment of \citet{moller_single-call_2011} and \E5{} is published in \citet{guse_modelling_2014}. However, these two experiments are omitted for modeling here.}
All other experiments are not considered suited for modeling, because only a very limited set of conditions was investigated.
...
This occurred, if more than two \ac{LP} episode/days were presented.
...
This indicates a longer integration interval than 3~usage episodes or, respectively, 3~days.
...
This allows to assign an individual weight to each episodic judgment and thus model the impact on a to be predicted multi-episodic judgment.
...
However, selecting a suiting weighting function is not a simple task, because overfitting is an issue.
...
All episodic judgments of usage episode in this window are assigned a weighting factor of 1 and all before a weighting factor of 0:
...
Here, the weight for usage episodes decreases linearly with an increasing distance to the multi-episodic judgment.
...
On the one hand, the weight of one of the three \ac{LP} episodes/days can be set to zero, and and the weight of \ac{HP} episodes increased.
...
Here, WW performs better while increasing $\mathit{w}=4$.
However, the prediction accuracy depends on the considered condition.
For example, \C3{} is far off for $\mathit{w}=1$ and improves until~3 while \C4{} is best predicted with~$\mathit{w}=5$.
...
It must be noted that \ac{RMSD} is in the best case close to \Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E1" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)} (linear, $\mathit{w}=2$).
...
In this case, LI is preferable over WW.
In fact, the WW is very precise for $\mathit{w}=4$, but is otherwise far off.
WI performs well for~\CVb{} with $\mathit{w} \geq 3$ and performs for all $\mathit{w}$ very similar for \C7{}.
...
Here, no difference between~WW and~LI is observed.
...
For WW a minimum is reached at $\mathit{w}=3$ and $\mathit{w}=4$ (\Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E2a" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_window_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)}).
For LI the minimal \ac{RMSD} is achieved at $\mathit{w}=2$ (\Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E2a" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)}).
...
For both experiments it is notably that the same parameters were found for the two weighting functions, although the usage situation is different.
...
In \E6{} a usage period of six~days was investigated for use of a \ac{AoD} service.
...
Here, increasing $\mathit{w}$ results, improves prediction accuracy.
This is more prevalent for WW than for LI.
WW achieves its minimal \ac{RMSD} with $\mathit{w}=6$ while LI provides only a slight decrease after $\mathit{w}=3$.
...
With regard to prediction the multi-episodic judgment after the 6th day, both weighting functions perform different.
\autoref{fig:pred:E6pred12} shows the \ac{RMSD} for both weighting functions.
While LI reaches a minimal \ac{RMSD} at $\mathit{w}=4$, WW achieves its minimal \ac{RMSD} not until $\mathit{w}=8$.
In addition, LI achieves a minimal \ac{RMSD} of \Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E6a" & data_nonadjusted[["id"]] == 14 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 1)} while WW only achieves a minimal \ac{RMSD} of
\Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E6a" & data_nonadjusted[["id"]] == 14 & data_nonadjusted[["model"]] == "weight_window_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)}.
With regard to \E6{}, LI is preferable to WW, because a higher prediction accuracy is achieved.
...
It is here proposed to replace the judgment of the first encountered \ac{LP} episode/day with the average of \ac{HP} episodic judgments.
This should resemble a sufficient approximation to account for the saturation effect.
This adjustment actually modifies \C6{} to be similar to \C5{}, which only differ in the performance level of the 4th episode/day.
...
ggplot_model_create(subset(data_adjusted_only, data_adjusted_only$experiment == "E1" & data_adjusted_only$id == 6), 1.25) + 
	geom_line(aes(x=parameter, y=RMSD, linetype=condition, size=condition, colour=condition)) + facet_grid(. ~ model_FACET) + 
...
In case of WW, the minimal \ac{RMSD} shifts from~$\mathit{w}=4$ to~$\mathit{w}=3$ for both experiments.
...
Furthermore, the minimal \ac{RMSD} is reached earlier, but that increasing $\mathit{w}$ further is worse for \C6{}~(adjusted) than \C6{}.
\begin{figure}[h]
	\centering
<<plotE2SAT, echo=F, fig.height=3>>=
ggplot_model_create(subset(data_adjusted_only, data_adjusted_only$experiment == "E2a" & data_adjusted_only$id == 6), 1.0) + 
	geom_line(aes(x=parameter, y=RMSD, linetype=condition, size=condition, colour=condition)) + facet_grid(. ~ model_FACET) + 
	scale_linetype_manual(name="Condition", values=c("solid", "twodash", "solid")) +
	scale_color_manual(name="Condition", values=c("#00BFC4", "#7CAE00", "#7CAE00")) +
	scale_size_manual(name="Condition", values=c(0.5, 0.5, 0.5))
...
ggplot_model_create(subset(data_adjusted_only, data_adjusted_only$experiment == "E6a" & data_adjusted_only$id == 14), 2.0) + 
	geom_line(aes(x=parameter, y=RMSD, linetype=condition, size=condition, colour=condition)) + facet_grid(. ~ model_FACET) + 
	scale_linetype_manual(name="Condition", values=c("solid", "twodash", "solid")) +
	scale_color_manual(name="Condition",values=c("#00BFC4", "#7CAE00", "#7CAE00")) +
	scale_size_manual(name="Condition", values=c(0.5, 0.5, 0.5))
...
A reduction in \ac{RMSD} with LI for all conditions is observed (\E1{}: 0.02, for \EIIa{} of 0.03, and for \E6{} of 0.08).
...
%round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E6a" & data_nonadjusted[["id"]] == 14 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)
%round(min(subset(data_adjusted, data_adjusted[["experiment"]] == "E6a" & data_adjusted[["id"]] == 14 & data_adjusted[["model"]] == "weight_linear_create" & data_adjusted[["condition"]] == "All")[["RMSD"]]), 2)

%round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E1" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)
%round(min(subset(data_adjusted, data_adjusted[["experiment"]] == "E1" & data_adjusted[["id"]] == 6 & data_adjusted[["model"]] == "weight_linear_create" & data_adjusted[["condition"]] == "All")[["RMSD"]]), 2)

%round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E2a" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)
%round(min(subset(data_adjusted, data_adjusted[["experiment"]] == "E2a" & data_adjusted[["id"]] == 6 & data_adjusted[["model"]] == "weight_linear_create" & data_adjusted[["condition"]] == "All")[["RMSD"]]), 2)


\section{Conclusion}
In this chapter, two model types based upon the weighted average for prediction multi-episodic perceived quality were presented.
...
Both model types perform similar, if only \ac{HP} usage episodes are presented.
In fact, if \ac{LP} usages episodes were presented, both models outperform the unweighted average.
Comparing WI and LI, the latter shows a prediction accuracy and higher robustness for choosing the parameter.
This is observed for all three experiments.
With regard to one-session experiments one interesting observation is made.
For both models, the highest prediction accuracy is achieved for the same $\mathit{w}$ in both experiments, \ie, LI: $\mathit{w}=2$ and WW: $\mathit{w}=4$.

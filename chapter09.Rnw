...
However, the weight of a usage episode decreases with the increasing distance to $n$.
...
However, for one observed effect a weighted average will necessarily produce deviations.

A modeling an explicit peak-effect is omitted here, because such an effect could not be observed in the conducted experiments.

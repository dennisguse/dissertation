...
data_adjusted_avg$model_FACET = factor(data_adjusted_avg$model == "weight_linear_create", labels=c("Window (WI)", "Linear (LI)")) #label and order; FALSE will be first; then attach labels for FACET
...
It must be noted that \ac{RMSD} is in the best case close to \Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E1" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)} (LI, $\mathit{w}=2$).
...
For WI a minimum is reached at $\mathit{w}=3$ and $\mathit{w}=4$ (\Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E2a" & data_nonadjusted[["id"]] == 6 & data_nonadjusted[["model"]] == "weight_window_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)}).
...
In addition, LI achieves a minimal \ac{RMSD} of \Sexpr{round(min(subset(data_nonadjusted, data_nonadjusted[["experiment"]] == "E6a" & data_nonadjusted[["id"]] == 14 & data_nonadjusted[["model"]] == "weight_linear_create" & data_nonadjusted[["condition"]] == "All")[["RMSD"]]), 1)} while WI only achieves a minimal \ac{RMSD} of
...
\begin{equation}\label{eq:saturate:modify}
\tilde{e}_3:= \left\{
\begin{array}{ll}
  \frac{e_6 + e_5 + e_4}{3},& \text{if episodes 1..3:LP and 4..6:HP}\\
  e_3,& \text{otherwise}
\end{array}
\right.
\end{equation}
...
This is equivalent to extending $m_n$ with the adjustment term:

...
However, the results are limited due to the small to no difference on multi\-/episodic judgments between the conditions.

This chapter starts with an overview of observed effects in the conducted experiments that seem relevant for prediction of multi\-/episodic judgments.
Following, model types are presented that seem suited for the prediction of multi\-/episodic judgments.
The prediction accuracy for those model types is evaluated using \E1{}, \EIIa{}, and \E6{}.
Those experiments yielded a large and in itself consistent data set.\footnote{An approach towards modeling based upon the experiment of \citet{moller_single-call_2011} and \E5{} is published in \citet{guse_modelling_2014}. However, these two experiments are omitted here for modeling due to the limited effects on multi\-/episodic judgments.}
All other here presented experiments are not considered suitable for modeling, because only a very limited set of conditions was investigated.
In difference to \citet{moller_single-call_2011}, who predicted multi\-/episodic judgments using episodic judgments per individual participant, the goal here is to predict the multi\-/episodic \ac{MOS} based upon the episodic \ac{MOS}.
...
Here, a decrease is observed until saturation occurred, \ie, multi\-/episodic judgments do not decrease further.
...
This indicates that the integration interval is longer than 3~usage episodes or 3~days, respectively.
Beside the integration interval, this indicates that the three \ac{LP} episodes/days had a reduced impact on the final multi\-/episodic judgment compared to the \ac{HP} episodes/days.
...
A peak effect is not considered for modeling, because such an effect could not be observed in the \E1{} (\autoref{hypo:strength}).
...
Here, nine episodes were presented while non-\ac{HP} episodes were presented after as the 5th and the \unit[6th]{episode}.
...
For one\-/session also a duration neglect was investigated (\autoref{hypo:duration}) in \E3{}. 
...
Therefore, it can be concluded that the duration of an episode without macroscopic fluctuations must not be accounted for prediction of multi\-/episodic judgments.
...
For implementation of a prediction model the modeling approach of \citet{moller_single-call_2011} is extended.
...
The weight for a episode is denoted as~$\mathit{a_i}$.
...
The weighted average model is in itself a rather simple model, because it is only parametrized with a weight function.
...
Here, two weight functions are proposed, which both can account for a recency effect.
...
All episodic judgments of usage episode in this window are assigned a weighting factor of 1 and all episodes before a weighting factor of~0 (\autoref{eq:weight:window}).
...
In fact, a static window is a rather unlikely case, because the importance of episodic judgments is considered for prediction only as binary.
To overcome this a linear weight function, denoted as \emph{LI}, is proposed here.
...
Here, presenting the 4th, 5th, and 6th\~episodes/days in \ac{LP} (\C6{}) results in a similar multi\-/episodic judgment than presenting only the 5th and 6th\,episodes/days in \ac{LP} while the 4th episode is presented in \ac{HP} (\C5{}).
The modeling approach for this specific case is discussed and evaluated in \autoref{pred:saturation}.
...
In the following, a \emph{model} denotes the combination of the selected weight function with the selected valud for the parameter~$\mathit{w}$.
...
Modeling is conducted using the episodic \ac{MOS} to predict the multi\-/episodic \ac{MOS}.
...
In \E1{} a two-party conversation was investigated with 6 to 9\,episodes.
...
LI performs well for~\CVb{} with $\mathit{w} \geq 3$ and performs in case of \C7{} very similar all for $\mathit{w}$.
Thus, LI is preferable, because it has a higher robustness.
...
With regard to prediction of \ac{HP} only episodes results are slightly different than for \E1{}.
...
However, this is due to strong deviation of~\C4{} alone, but the reason for this could not be determined.
Omitting \C4{} shows similar results to~\E1{}, \ie, prediction accuracy is not affected by $\mathit{w}$.
In fact, \ac{RMSD} reaches a similar value than for~\E1{}, \ie, 0.1, for all $\mathit{w}$.
...
For WI a minimum is reached at $\mathit{w}=4$ (\Sexpr{round(min(subset(data_mos_nonadjusted, data_mos_nonadjusted[["experiment"]] == "E2a" & data_mos_nonadjusted[["id"]] == 6 & data_mos_nonadjusted[["model"]] == "weight_window_create" & data_mos_nonadjusted[["condition"]] == "All")[["RMSD"]]), 2)}) while $\mathit{w}=3$ is rather close.
...
For both experiments it is notable that imilar values for the $\mathit{w}$ were found for each of the two weight functions although the usage situation is different.
With regard to prediction accuracy LI is preferable over~WI.
Besides, LI seems to be more robust against improper selection of $\mathit{w}$.
For both experiments a minimal \ac{RMSD} could be achieved for $\mathit{w}=2$ for LI.
For predicting the multi\-/episodic judgments after the 9th\,episode, which has only been investigated in \E1{}, LI ($\mathit{w}=3$) performs best. 
...
Here, the prediction accuracy is improves by for an increasing $\mathit{w}$.
This is more prevalent for WI than for LI.
WI achieves its minimal \ac{RMSD} with $\mathit{w}=6$, \ie, all prior episodes, while LI provides only a slight decrease after $\mathit{w}=3$.
...
#	scale_size_manual(name="Condition", values=c(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 1))
@
	\caption[Multiple days (\E6{}): prediction accuracy for \ac{HP} only episodes]{Multiple days (\E6{}): prediction accuracy for \ac{HP} only episodes (3rd day, \ie, 6th usage episode).}
	\label{fig:pred:E6pred6}
\end{figure}

With regard to prediction the multi\-/episodic judgment after the 6th\,day both weight functions perform differently.
...
#	scale_size_manual(name="Condition", values=c(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 1))
@
	\caption[Multiple days (\E6{}): prediction accuracy for all conditions]{Multiple days (\E6{}): prediction accuracy for all conditions (6th day, \ie, 12th usage episode).}
...
Here, multi\-/episodic judgments remain on the same level independent if two or three \ac{LP} episodes/days are presented, \ie, \C5{} and \C6{} are judged not different.
In both cases the multi-episodic judgment remains above the episodic judgment for \ac{LP} episodes (approx.\,~\unit[1]{pt}).
...
With regard to the applicability of a weighted average model this is problematic, because a model needs to produce a similar prediction based upon different input, \ie, one \ac{HP} episode/day and two \ac{LP} episodes/days (\C5{}) versus three \ac{LP} episodes/days (\C6{}).
As the multi-episodic judgment remains above the level of episodic judgments of \ac{LP} episodes, at least the last four episodes/days must be considered in the case of \C6{} while \C5{} only requires three episodes/days.
For all three experiments LI and WW require a larger $\mathit{w}$ to reach the best prediction accuracy for \C6{} compared to all other conditions.
For \E1{} and \EIIa{} $\mathit{w}=3$~(LI) and $\mathit{w}=4$~(WW) are required for \C6{} while \C5{} achieves its minimum already at $\mathit{w}=2$~(LI) and $\mathit{w}=3$~(WW).
For \E6{} $\mathit{w}=7$~(LI) and $\mathit{w}=9$~(WW) are required for \C6{} while \C5{} achieves its minimum at $\mathit{w}=4$~(LI) and $\mathit{w}=7$~(WW).
It must be noted that the optimal value of $\mathit{w}$ for \C5{} is identical to the smallest near optimum values for $\mathit{w}$ in case of LI and WI.
Thus, the overall prediction performance for both weighting functions can be improved if $\mathit{w}$ can be reduced to this value in case of \C6{}.
...
The modified version of \C6{} is in the following denoted as \C6{}~(adjusted).
...
@
	\caption[One session (\E1{}): prediction accuracy for saturation effect]{One session (\E1{}): prediction accuracy for saturation effect (6th usage episode).}
...
ggplot_model_create(subset(data_mos_adjusted_only, data_mos_adjusted_only$experiment == "E2a" & data_mos_adjusted_only$id == 6), 1.0) + 
	...
	scale_size_manual(name="Condition", values=c(0.5, 0.5, 0.5))
@
	\caption[One session (\EIIa{}): prediction accuracy for saturation effect]{One session (\EIIa{}): prediction accuracy for saturation effect (6th usage episode).}
...
ggplot_model_create(subset(data_mos_adjusted_only, data_mos_adjusted_only$experiment == "E6a" & data_mos_adjusted_only$id == 14), 2.0) + 
	...
	scale_size_manual(name="Condition", values=c(0.5, 0.5, 0.5))
@
	\caption[Multiple days (\E6{}): prediction accuracy for saturation effect]{Multiple days (\E6{}): prediction accuracy for saturation effect (6th day, \ie, 6th usage episode).}
...
Although the \ac{RMSD} is increased in some cases, $\mathit{w}$ is reduced to the optimal solution found for all other conditions without saturation adjustment (see \autoref{results:prediction}).
For all conditions this leads to an reduction of \ac{RMSD} (\E1{}: 0.02, \EIIa{}: 0.03, and \E6{}: 0.08).
In fact, this overall reduction is rather small and it can thus be concluded that, although accounting for saturation improves prediction accuracy, this improvement is rather small considering all conditions.
...
\section{Conclusion}
In this chapter two model types based upon the weighted average for predicting the multi\-/episodic \ac{MOS} using the episodic \acp{MOS} were presented.
...
Comparing WI and LI the latter shows a better prediction accuracy and higher robustness for choosing the $\mathit{w}$ as observed for all three experiments.
One interesting observation is made with regard to one-session experiments.
For both models, the highest prediction accuracy is achieved for the same $\mathit{w}$ for both experiments, \ie, LI:~$\mathit{w}=2$ and WI:~$\mathit{w}=4$.
...
In fact, it must be noted that the weighted average using rather simple weight functions with one degree of freedom enables a decent prediction accuracy for one-session as well as a usage period of 6~days.
